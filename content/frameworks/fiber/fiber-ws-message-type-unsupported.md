---
title: "WebSocket message type unsupported"
description: "Fiber WebSocket handler receives a binary message type but the handler only processes text frames causing the message to be silently dropped"
frameworks: ['fiber']
...
This error occurs when fiber websocket handler receives a binary message type but the handler only processes text frames causing the message to be silently dropped.
