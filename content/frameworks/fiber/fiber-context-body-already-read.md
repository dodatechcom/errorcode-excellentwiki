---
title: "Context body already consumed"
description: "Fiber handler calls ctx.Body() multiple times but the second call returns an empty slice because the fasthttp body buffer is already consumed"
frameworks: ['fiber']
...
This error occurs when fiber handler calls ctx.body() multiple times but the second call returns an empty slice because the fasthttp body buffer is already consumed.
