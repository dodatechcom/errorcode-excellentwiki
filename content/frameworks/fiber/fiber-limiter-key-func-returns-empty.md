---
title: "Rate limiter key function returns empty"
description: "Fiber rate limiter groups all requests under a single empty key instead of per-IP because the KeyGenerator function returns an empty string"
frameworks: ['fiber']
...
This error occurs when fiber rate limiter groups all requests under a single empty key instead of per-ip because the keygenerator function returns an empty string.
