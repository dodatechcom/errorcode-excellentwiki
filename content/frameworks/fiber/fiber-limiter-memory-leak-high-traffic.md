---
title: "Rate limiter memory leak under high traffic"
description: "Fiber sliding window rate limiter stores per-IP counters without expiration under high traffic causing the server to run out of memory"
frameworks: ['fiber']
...
This error occurs when fiber sliding window rate limiter stores per-ip counters without expiration under high traffic causing the server to run out of memory.
