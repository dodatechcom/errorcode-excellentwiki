---
title: "In-memory rate limiter exhaustion"
description: "Actix in-memory rate limiter stores per-IP counters indefinitely under high traffic causing the server process to run out of memory"
frameworks: ['actix']
...
This error occurs when actix in-memory rate limiter stores per-ip counters indefinitely under high traffic causing the server process to run out of memory.
