---
title: "HTTP/2 flow control stall"
description: "Actix HTTP/2 server stalls on large response bodies because the client does not consume the stream fast enough and flow control windows fill"
frameworks: ['actix']
...
This error occurs when actix http/2 server stalls on large response bodies because the client does not consume the stream fast enough and flow control windows fill.
