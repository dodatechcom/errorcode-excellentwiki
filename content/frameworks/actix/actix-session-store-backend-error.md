---
title: "Session store backend error"
description: "Actix session middleware fails to persist the session because the Redis or database backend connection pool is exhausted"
frameworks: ['actix']
...
This error occurs when actix session middleware fails to persist the session because the redis or database backend connection pool is exhausted.
