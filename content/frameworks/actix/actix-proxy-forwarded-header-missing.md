---
title: "Proxy Forwarded header missing"
description: "Actix server behind a reverse proxy reads the wrong client IP because the Forwarded header is not configured to be trusted"
frameworks: ['actix']
...
This error occurs when actix server behind a reverse proxy reads the wrong client ip because the forwarded header is not configured to be trusted.
