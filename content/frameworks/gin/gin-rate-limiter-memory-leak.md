...
description: "Gin rate limiter middleware stores per-IP entries indefinitely without expiration causing unbounded memory growth over time"
frameworks: ['gin']
...
This error occurs when gin rate limiter middleware stores per-ip entries indefinitely without expiration causing unbounded memory growth over time.
