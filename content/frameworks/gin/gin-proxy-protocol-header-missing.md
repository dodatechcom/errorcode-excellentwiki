---
title: "Proxy protocol header missing"
description: "Gin server behind a load balancer reads an incorrect client IP because the PROXY protocol header is not forwarded by the proxy"
frameworks: ['gin']
...
This error occurs when gin server behind a load balancer reads an incorrect client ip because the proxy protocol header is not forwarded by the proxy.
