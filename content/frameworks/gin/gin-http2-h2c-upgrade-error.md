---
title: "HTTP/2 h2c upgrade error"
description: "Gin HTTP/2 cleartext upgrade fails because the handler does not check for the h2c Upgrade header before processing the request"
frameworks: ['gin']
...
This error occurs when gin http/2 cleartext upgrade fails because the handler does not check for the h2c upgrade header before processing the request.
