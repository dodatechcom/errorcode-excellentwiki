---
title: "WebSocket upgrade request failed"
description: "Gin WebSocket handler cannot upgrade the HTTP connection because the required Upgrade headers are missing or the protocol is rejected"
frameworks: ['gin']
...
This error occurs when gin websocket handler cannot upgrade the http connection because the required upgrade headers are missing or the protocol is rejected.
