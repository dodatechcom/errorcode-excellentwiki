---
title: "[Solution] Cassandra - hints not enabled"
description: "Understand and resolve the Cassandra 'hints not enabled' error with causes, CQL fixes, and examples."
tools: ["cassandra"]
...
# Cassandra - hints not enabled

The Cassandra `hints not enabled` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `hints not enabled` error in Cassandra indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `hints not enabled` error in Cassandra:
