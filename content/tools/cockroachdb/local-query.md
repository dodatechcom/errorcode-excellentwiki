---
title: "[Solution] CockroachDB - local query"
description: "Understand and resolve the CockroachDB 'local query' error with causes, SQL fixes, and examples."
tools: ["cockroachdb"]
...
# CockroachDB - local query

The CockroachDB `local query` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `local query` error in CockroachDB indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `local query` error in CockroachDB:
