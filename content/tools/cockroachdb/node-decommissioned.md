---
title: "[Solution] CockroachDB - node decommissioned"
description: "Understand and resolve the CockroachDB 'node decommissioned' error with causes, SQL fixes, and examples."
tools: ["cockroachdb"]
...
# CockroachDB - node decommissioned

The CockroachDB `node decommissioned` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `node decommissioned` error in CockroachDB indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `node decommissioned` error in CockroachDB:
