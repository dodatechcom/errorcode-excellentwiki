---
title: "[Solution] CockroachDB - resolved timestamp"
description: "Understand and resolve the CockroachDB 'resolved timestamp' error with causes, SQL fixes, and examples."
tools: ["cockroachdb"]
...
# CockroachDB - resolved timestamp

The CockroachDB `resolved timestamp` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `resolved timestamp` error in CockroachDB indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `resolved timestamp` error in CockroachDB:
