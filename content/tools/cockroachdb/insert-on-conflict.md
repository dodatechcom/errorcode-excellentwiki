---
title: "[Solution] CockroachDB - INSERT ON CONFLICT"
description: "Understand and resolve the CockroachDB 'INSERT ON CONFLICT' error with causes, SQL fixes, and examples."
tools: ["cockroachdb"]
...
# CockroachDB - INSERT ON CONFLICT

The CockroachDB `INSERT ON CONFLICT` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `INSERT ON CONFLICT` error in CockroachDB indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `INSERT ON CONFLICT` error in CockroachDB:
