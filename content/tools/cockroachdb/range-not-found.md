---
title: "[Solution] CockroachDB - range not found"
description: "Understand and resolve the CockroachDB 'range not found' error with causes, SQL fixes, and examples."
tools: ["cockroachdb"]
...
# CockroachDB - range not found

The CockroachDB `range not found` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `range not found` error in CockroachDB indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `range not found` error in CockroachDB:
