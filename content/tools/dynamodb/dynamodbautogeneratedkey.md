---
title: "[Solution] DynamoDB - @DynamoDBAutoGeneratedKey"
description: "Understand and resolve the DynamoDB '@DynamoDBAutoGeneratedKey' error with causes, AWS CLI and SDK fixes, and examples."
tools: ["dynamodb"]
...
# DynamoDB - @DynamoDBAutoGeneratedKey

The DynamoDB `@DynamoDBAutoGeneratedKey` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `@DynamoDBAutoGeneratedKey` error in DynamoDB indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `@DynamoDBAutoGeneratedKey` error in DynamoDB:
