---
title: "[Solution] DynamoDB - scaling policy"
description: "Understand and resolve the DynamoDB 'scaling policy' error with causes, AWS CLI and SDK fixes, and examples."
tools: ["dynamodb"]
...
The DynamoDB `scaling policy` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `scaling policy` error in DynamoDB indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `scaling policy` error in DynamoDB:
