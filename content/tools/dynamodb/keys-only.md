---
title: "[Solution] DynamoDB - keys only"
description: "Understand and resolve the DynamoDB 'keys only' error with causes, AWS CLI and SDK fixes, and examples."
tools: ["dynamodb"]
...
The DynamoDB `keys only` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `keys only` error in DynamoDB indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `keys only` error in DynamoDB:
