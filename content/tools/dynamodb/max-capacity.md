---
title: "[Solution] DynamoDB - max capacity"
description: "Understand and resolve the DynamoDB 'max capacity' error with causes, AWS CLI and SDK fixes, and examples."
tools: ["dynamodb"]
...
# DynamoDB - max capacity

The DynamoDB `max capacity` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `max capacity` error in DynamoDB indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `max capacity` error in DynamoDB:
