---
title: "[Solution] DynamoDB - scan timeout"
description: "Understand and resolve the DynamoDB 'scan timeout' error with causes, AWS CLI and SDK fixes, and examples."
tools: ["dynamodb"]
...
The DynamoDB `scan timeout` error can occur during database operations. This page explains what causes it and how to resolve it.

## What This Error Means

Encountering the `scan timeout` error in DynamoDB indicates a problem that prevents normal database operations from completing successfully. Identifying the root cause quickly is key to minimizing downtime.
...
A typical occurrence of the `scan timeout` error in DynamoDB:
