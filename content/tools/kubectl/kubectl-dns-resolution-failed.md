---
title: "[Solution] Kubectl DNS Resolution Failed"
description: "Fix Kubernetes DNS resolution failures. Debug CoreDNS configuration and service discovery."
tools: ["kubectl"]
...
The `kubectl` command encountered a **DNSResolutionFailed** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get dns failed --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-dns-failed --image=nginx --restart=Never --dry-run=client -o yaml
```
