---
title: "[Solution] Kubectl Cluster Unreachable"
description: "Fix Kubernetes cluster unreachable errors. Debug network, certificates, and API server connectivity."
tools: ["kubectl"]
...
The `kubectl` command encountered a **ClusterUnreachable** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get cluster unreachable --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-cluster-unreachable --image=nginx --restart=Never --dry-run=client -o yaml
```
