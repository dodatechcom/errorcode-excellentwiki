---
title: "[Solution] Kubectl OOMKilled"
description: "Fix Kubernetes OOMKilled errors. Increase container memory limits and resolve memory exhaustion."
tools: ["kubectl"]
...
The `kubectl` command encountered a **OOMKilled** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get oom killed --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-oom-killed --image=nginx --restart=Never --dry-run=client -o yaml
```
