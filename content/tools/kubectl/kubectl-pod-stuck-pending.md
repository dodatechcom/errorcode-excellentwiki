---
title: "[Solution] Kubectl Pod Stuck Pending"
description: "Fix Kubernetes pods stuck in Pending state. Resolve scheduling failures and resource constraints."
tools: ["kubectl"]
...
The `kubectl` command encountered a **PodPending** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get pod pending --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-pod-pending --image=nginx --restart=Never --dry-run=client -o yaml
```
