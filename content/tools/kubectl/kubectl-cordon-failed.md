---
title: "[Solution] Kubectl Cordon Failed"
description: "Fix Kubernetes cordon failures. Debug node cordoning and maintenance mode issues."
tools: ["kubectl"]
...
The `kubectl` command encountered a **CordonFailed** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get cordon failed --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-cordon-failed --image=nginx --restart=Never --dry-run=client -o yaml
```
