---
title: "[Solution] Kubectl Verb Not Allowed"
description: "Fix Kubernetes verb not allowed errors. Update RBAC permissions for kubectl operations."
tools: ["kubectl"]
...
The `kubectl` command encountered a **VerbNotAllowed** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get verb not allowed --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-verb-not-allowed --image=nginx --restart=Never --dry-run=client -o yaml
```
