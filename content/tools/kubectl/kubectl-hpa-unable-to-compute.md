---
title: "[Solution] Kubectl HPA Unable to Compute"
description: "Fix Kubernetes HPA unable to compute metrics errors. Configure metrics server and resource requests."
tools: ["kubectl"]
...
The `kubectl` command encountered a **HPAUnableToCompute** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get hpa error --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-hpa-error --image=nginx --restart=Never --dry-run=client -o yaml
```
