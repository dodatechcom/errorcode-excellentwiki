---
title: "[Solution] Kubectl Service Unreachable"
description: "Fix Kubernetes service unreachable errors. Debug service discovery, endpoints, and network policies."
tools: ["kubectl"]
...
The `kubectl` command encountered a **ServiceUnreachable** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get service unreachable --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-service-unreachable --image=nginx --restart=Never --dry-run=client -o yaml
```
