...
description: "Fix Kubernetes pod log retrieval failures. Debug logging configuration and container states."
tools: ["kubectl"]
...
The `kubectl` command encountered a **LogsFailed** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get logs failed --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-logs-failed --image=nginx --restart=Never --dry-run=client -o yaml
```
