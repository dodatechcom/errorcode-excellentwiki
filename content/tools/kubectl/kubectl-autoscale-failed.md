---
title: "[Solution] Kubectl Autoscale Failed"
description: "Fix Kubernetes autoscale failures. Configure HPA and debug horizontal pod autoscaling."
tools: ["kubectl"]
...
The `kubectl` command encountered a **AutoscaleFailed** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get autoscale failed --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-autoscale-failed --image=nginx --restart=Never --dry-run=client -o yaml
```
