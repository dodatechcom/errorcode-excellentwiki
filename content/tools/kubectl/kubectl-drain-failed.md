...
description: "Fix Kubernetes drain failures. Handle pod eviction errors and graceful node draining."
tools: ["kubectl"]
...
The `kubectl` command encountered a **DrainFailed** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get drain failed --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-drain-failed --image=nginx --restart=Never --dry-run=client -o yaml
```
