---
title: "[Solution] Kubectl Copy Failed"
description: "Fix Kubernetes pod copy failures. Debug file transfer issues between local machine and pods."
tools: ["kubectl"]
...
The `kubectl` command encountered a **CopyFailed** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get copy failed --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-copy-failed --image=nginx --restart=Never --dry-run=client -o yaml
```
