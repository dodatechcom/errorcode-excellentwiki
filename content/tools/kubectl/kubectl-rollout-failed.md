---
title: "[Solution] Kubectl Rollout Failed"
description: "Fix Kubernetes rollout failures. Debug deployment rollouts, rollbacks, and status tracking."
tools: ["kubectl"]
...
The `kubectl` command encountered a **RolloutFailed** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get rollout failed --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-rollout-failed --image=nginx --restart=Never --dry-run=client -o yaml
```
