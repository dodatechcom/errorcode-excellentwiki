...
description: "Fix Kubernetes port forward failures. Debug port conflicts, pod readiness, and network issues."
tools: ["kubectl"]
...
The `kubectl` command encountered a **PortForwardFailed** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get port forward --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-port-forward --image=nginx --restart=Never --dry-run=client -o yaml
```
