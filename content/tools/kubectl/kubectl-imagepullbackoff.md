---
title: "[Solution] Kubectl ImagePullBackOff"
description: "Fix Kubernetes ImagePullBackOff errors. Resolve image pull failures with common causes and solutions."
tools: ["kubectl"]
...
The `kubectl` command encountered a **ImagePullBackOff** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get image pull backoff --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-image-pull-backoff --image=nginx --restart=Never --dry-run=client -o yaml
```
