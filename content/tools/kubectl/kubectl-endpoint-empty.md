---
title: "[Solution] Kubectl Endpoint Empty"
description: "Fix Kubernetes empty endpoint errors. Debug service selector mismatches and pod readiness probes."
tools: ["kubectl"]
...
The `kubectl` command encountered a **EndpointEmpty** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get endpoint empty --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-endpoint-empty --image=nginx --restart=Never --dry-run=client -o yaml
```
