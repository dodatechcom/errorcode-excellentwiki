---
title: "[Solution] Kubectl Uncordon Failed"
description: "Fix Kubernetes uncordon failures. Restore nodes to schedulable state after maintenance."
tools: ["kubectl"]
...
The `kubectl` command encountered a **UncordonFailed** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get uncordon failed --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-uncordon-failed --image=nginx --restart=Never --dry-run=client -o yaml
```
