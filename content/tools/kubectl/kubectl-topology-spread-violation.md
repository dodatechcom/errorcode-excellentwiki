---
title: "[Solution] Kubectl Topology Spread Violation"
description: "Fix Kubernetes topology spread constraint violations. Balance pod distribution across zones."
tools: ["kubectl"]
...
The `kubectl` command encountered a **TopologySpreadViolation** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
kubectl get topology violation --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-topology-violation --image=nginx --restart=Never --dry-run=client -o yaml
```
