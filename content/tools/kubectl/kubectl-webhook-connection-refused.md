---
title: "[Solution] Kubectl Webhook Connection Refused"
description: "Fix Kubernetes admission webhook connection refused. Debug webhook configurations and certificates."
tools: ["kubectl"]
...
The `kubectl` command encountered a **WebhookConnectionRefused** issue. This error stops normal operation and must be resolved before continuing with your workflow.
...
```
Error: WebhookConnectionRefused
```
...
kubectl get webhook refused --all-namespaces 2>/dev/null || true
```
...
```bash
kubectl run test-webhook-refused --image=nginx --restart=Never --dry-run=client -o yaml
```
