---
title: "[Solution] Linux: zfs-send-receive-error — ZFS send/receive error"
description: "Fix Linux zfs-send-receive-error errors. ZFS send/receive error with these solutions."
platforms: ["linux"]
severities: ["warning"]
...
# Linux: ZFS Send Receive Error Error

ZFS send receive error errors occur when the ZFS filesystem encounters pool, dataset, or data integrity issues.
...
```bash
sudo zfs get all tank | grep -i "send-receive-error"
```
