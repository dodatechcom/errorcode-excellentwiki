...
For files that appear partially recoverable, use specialized recovery tools:
...
# Or use PhotoRec for specific file types
photorec /path/to/disk
```

### 5. Validate File Format in Code

Add validation before attempting to parse file contents:
...
- Opening a document that was saved by an application that crashed mid-write
