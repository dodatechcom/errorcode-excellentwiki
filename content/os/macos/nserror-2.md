...
NSFileWriteUnknownError (error code 513 in NSCocoaErrorDomain) indicates that an unspecified error occurred while attempting to write a file. This generic error provides no specific reason for the failure, requiring investigation into disk space, permissions, file system integrity, and sandbox restrictions.
...
- A sandbox restriction prevents the application from writing to the location
...
Verify there is enough space on the target volume:
...
### 3. Check for Volume-Level Issues
...
### 5. Use Explicit Error Logging in Code

Log detailed error information to diagnose the root cause:
...
- Writing to a network share that has become disconnected mid-operation
