---
title: "[Solution] Error api-ms-win-crt-utility Windows 11/10 — Api Ms Win Crt Utility Fixed"
description: "Learn how to fix Windows error api-ms-win-crt-utility (Api Ms Win Crt Utility) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["runtime"]
weight: 10
...
# [Solution] Error api-ms-win-crt-utility Windows 11/10 — Api Ms Win Crt Utility Fixed

The api-ms-win-crt-utility-l1-1-0.dll is missing causing utility functions such as environment access and error handling to fail. Reinstall the Universal C Runtime to restore this missing DLL file.

Despite its technical name, error api-ms-win-crt-utility typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error api-ms-win-crt-utility (Api Ms Win Crt Utility) is a Windows system error that appears in the Dll category. The full message usually reads:

> "Error api-ms-win-crt-utility: Api Ms Win Crt Utility."
...
Most DLL errors related to Api Ms Win Crt Utility are resolved by reinstalling the Visual C++ runtimes:
...
```cmd
regsvr32 /u api_ms_win_crt_utility.dll
regsvr32 api_ms_win_crt_utility.dll
```
