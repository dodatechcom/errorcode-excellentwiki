---
title: "[Solution] Error msvcr120-dll-error Windows 11/10 — Msvcr120 Dll Fixed"
description: "Learn how to fix Windows error msvcr120-dll-error (Msvcr120 Dll) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["runtime"]
weight: 10
...
# [Solution] Error msvcr120-dll-error Windows 11/10 — Msvcr120 Dll Fixed

The MSVCR120.dll file is missing or corrupted causing applications that require Visual C++ 2013 runtime to fail. Install both x86 and x64 versions of the Visual C++ 2013 Redistributable package.

Despite its technical name, error msvcr120-dll-error typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error msvcr120-dll-error (Msvcr120 Dll) is a Windows system error that appears in the Dll category. The full message usually reads:

> "Error msvcr120-dll-error: Msvcr120 Dll."
...
Most DLL errors related to Msvcr120 Dll are resolved by reinstalling the Visual C++ runtimes:
...
```cmd
regsvr32 /u msvcr120_dll_error.dll
regsvr32 msvcr120_dll_error.dll
```
