---
title: "[Solution] Error bad-system-config-info Windows 11/10 — Bad System Config Info Fixed"
description: "Learn how to fix Windows error bad-system-config-info (Bad System Config Info) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["kernel"]
weight: 10
...
# [Solution] Error bad-system-config-info Windows 11/10 — Bad System Config Info Fixed

Windows detected incorrect or corrupted system configuration information in the registry or boot configuration data. This error can prevent the system from booting and requires recovery environment intervention.

Despite its technical name, error bad-system-config-info typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error bad-system-config-info (Bad System Config Info) is a Windows system error that appears in the Modern Bsod category. The full message usually reads:

> "Error bad-system-config-info: Bad System Config Info."
