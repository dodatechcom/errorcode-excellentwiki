---
title: "[Solution] Error msvcp140-dll-error Windows 11/10 — Msvcp140 Dll Fixed"
description: "Learn how to fix Windows error msvcp140-dll-error (Msvcp140 Dll) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["runtime"]
weight: 10
...
# [Solution] Error msvcp140-dll-error Windows 11/10 — Msvcp140 Dll Fixed

The MSVCP140.dll file is missing or corrupted causing applications to fail on startup with a dynamic link library error. This file is part of the Microsoft Visual C++ 2015-2022 runtime required by many modern Windows applications.

Despite its technical name, error msvcp140-dll-error typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error msvcp140-dll-error (Msvcp140 Dll) is a Windows system error that appears in the Dll category. The full message usually reads:

> "Error msvcp140-dll-error: Msvcp140 Dll."
...
Most DLL errors related to Msvcp140 Dll are resolved by reinstalling the Visual C++ runtimes:
...
```cmd
regsvr32 /u msvcp140_dll_error.dll
regsvr32 msvcp140_dll_error.dll
```
