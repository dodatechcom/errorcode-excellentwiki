---
title: "[Solution] Error api-ms-win-crt-io Windows 11/10 — Api Ms Win Crt Io Fixed"
description: "Learn how to fix Windows error api-ms-win-crt-io (Api Ms Win Crt Io) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["runtime"]
weight: 10
...
# [Solution] Error api-ms-win-crt-io Windows 11/10 — Api Ms Win Crt Io Fixed

The api-ms-win-crt-io-l1-1-0.dll file is missing or corrupted preventing file I/O operations in C runtime applications. This error is resolved by installing or repairing the Universal C Runtime component.

Despite its technical name, error api-ms-win-crt-io typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error api-ms-win-crt-io (Api Ms Win Crt Io) is a Windows system error that appears in the Dll category. The full message usually reads:

> "Error api-ms-win-crt-io: Api Ms Win Crt Io."
...
Most DLL errors related to Api Ms Win Crt Io are resolved by reinstalling the Visual C++ runtimes:
...
```cmd
regsvr32 /u api_ms_win_crt_io.dll
regsvr32 api_ms_win_crt_io.dll
```
