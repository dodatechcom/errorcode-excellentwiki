---
title: "[Solution] Error attempted-write-to-readonly-memory Windows 11/10 — Attempted Write To Readonly Memory Fixed"
description: "Learn how to fix Windows error attempted-write-to-readonly-memory (Attempted Write To Readonly Memory) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["kernel"]
weight: 10
...
# [Solution] Error attempted-write-to-readonly-memory Windows 11/10 — Attempted Write To Readonly Memory Fixed

A kernel-mode driver attempted to modify a read-only section of system memory causing a blue screen. This error indicates a serious bug in a device driver that is writing to memory it does not own.

Despite its technical name, error attempted-write-to-readonly-memory typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error attempted-write-to-readonly-memory (Attempted Write To Readonly Memory) is a Windows system error that appears in the Modern Bsod category. The full message usually reads:

> "Error attempted-write-to-readonly-memory: Attempted Write To Readonly Memory."
