---
title: "[Solution] Error no-such-device Windows 11/10 — No Such Device Fixed"
description: "Learn how to fix Windows error no-such-device (No Such Device) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["kernel"]
weight: 10
...
# [Solution] Error no-such-device Windows 11/10 — No Such Device Fixed

The system encountered a reference to a device that no longer exists or is not accessible. This error commonly occurs when external storage devices are disconnected during operations or when drivers reference removed hardware.

Despite its technical name, error no-such-device typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error no-such-device (No Such Device) is a Windows system error that appears in the Modern Bsod category. The full message usually reads:
