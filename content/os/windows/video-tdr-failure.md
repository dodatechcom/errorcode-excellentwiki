---
title: "[Solution] Error video-tdr-failure Windows 11/10 — Video Tdr Failure Fixed"
description: "Learn how to fix Windows error video-tdr-failure (Video Tdr Failure) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["kernel"]
weight: 10
...
# [Solution] Error video-tdr-failure Windows 11/10 — Video Tdr Failure Fixed

The display driver stopped responding and the Timeout Detection Recovery mechanism failed to restart it. This blue screen error is commonly caused by GPU overheating, overclocking, or unstable graphics drivers.

Despite its technical name, error video-tdr-failure typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error video-tdr-failure (Video Tdr Failure) is a Windows system error that appears in the Modern Bsod category. The full message usually reads:

> "Error video-tdr-failure: Video Tdr Failure."
