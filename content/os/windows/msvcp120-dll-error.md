---
title: "[Solution] Error msvcp120-dll-error Windows 11/10 — Msvcp120 Dll Fixed"
description: "Learn how to fix Windows error msvcp120-dll-error (Msvcp120 Dll) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["runtime"]
weight: 10
...
# [Solution] Error msvcp120-dll-error Windows 11/10 — Msvcp120 Dll Fixed

The MSVCP120.dll file is missing or corrupted causing applications that depend on Visual C++ 2013 C++ runtime to fail. Install the Microsoft Visual C++ 2013 Redistributable to restore this DLL file.

Despite its technical name, error msvcp120-dll-error typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error msvcp120-dll-error (Msvcp120 Dll) is a Windows system error that appears in the Dll category. The full message usually reads:

> "Error msvcp120-dll-error: Msvcp120 Dll."
...
Most DLL errors related to Msvcp120 Dll are resolved by reinstalling the Visual C++ runtimes:
...
```cmd
regsvr32 /u msvcp120_dll_error.dll
regsvr32 msvcp120_dll_error.dll
```
