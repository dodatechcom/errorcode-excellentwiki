---
title: "[Solution] Error 0x800706be Windows 11/10 — Store Rpc Failed Fixed"
description: "Learn how to fix Windows error 0x800706be (Store Rpc Failed) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["runtime"]
weight: 10
...
# [Solution] Error 0x800706be Windows 11/10 — Store Rpc Failed Fixed

Microsoft Store encountered a remote procedure call failure communicating with system services. Restart the Windows Update and Store services to restore the RPC communication pathway.

Despite its technical name, error 0x800706be typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error 0x800706be (Store Rpc Failed) is a Windows system error that appears in the Store category. The full message usually reads:

> "Error 0x800706be: Store Rpc Failed."
