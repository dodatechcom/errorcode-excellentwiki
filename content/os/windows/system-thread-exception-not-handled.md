---
title: "[Solution] Error system-thread-exception-not-handled Windows 11/10 — System Thread Exception Not Handled Fixed"
description: "Learn how to fix Windows error system-thread-exception-not-handled (System Thread Exception Not Handled) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["kernel"]
weight: 10
...
# [Solution] Error system-thread-exception-not-handled Windows 11/10 — System Thread Exception Not Handled Fixed

A system thread running in kernel mode raised an exception that was not handled by the Windows error handler. This blue screen is typically caused by faulty device drivers or corrupted system files.

Despite its technical name, error system-thread-exception-not-handled typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error system-thread-exception-not-handled (System Thread Exception Not Handled) is a Windows system error that appears in the Modern Bsod category. The full message usually reads:

> "Error system-thread-exception-not-handled: System Thread Exception Not Handled."
