---
title: "[Solution] Error api-ms-win-crt-string Windows 11/10 — Api Ms Win Crt String Fixed"
description: "Learn how to fix Windows error api-ms-win-crt-string (Api Ms Win Crt String) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["runtime"]
weight: 10
...
# [Solution] Error api-ms-win-crt-string Windows 11/10 — Api Ms Win Crt String Fixed

The api-ms-win-crt-string-l1-1-0.dll file is missing or corrupted preventing string manipulation functions from working. Applications built with the Universal C Runtime require this DLL to operate correctly.

Despite its technical name, error api-ms-win-crt-string typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error api-ms-win-crt-string (Api Ms Win Crt String) is a Windows system error that appears in the Dll category. The full message usually reads:

> "Error api-ms-win-crt-string: Api Ms Win Crt String."
...
Most DLL errors related to Api Ms Win Crt String are resolved by reinstalling the Visual C++ runtimes:
...
```cmd
regsvr32 /u api_ms_win_crt_string.dll
regsvr32 api_ms_win_crt_string.dll
```
