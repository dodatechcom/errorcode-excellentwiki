---
title: "[Solution] Error api-ms-win-crt-math Windows 11/10 — Api Ms Win Crt Math Fixed"
description: "Learn how to fix Windows error api-ms-win-crt-math (Api Ms Win Crt Math) on Windows 10 and 11. Follow these proven step-by-step solutions with copyable commands."
platforms: ["windows"]
severities: ["error"]
error_types: ["runtime"]
weight: 10
...
# [Solution] Error api-ms-win-crt-math Windows 11/10 — Api Ms Win Crt Math Fixed

The api-ms-win-crt-math-l1-1-0.dll is missing causing mathematical computation functions to fail in dependent applications. Install the latest Visual C++ Redistributable package to restore this runtime component.

Despite its technical name, error api-ms-win-crt-math typically points to underlying system issues that can be resolved with built-in Windows tools and systematic troubleshooting.

## Description

Error api-ms-win-crt-math (Api Ms Win Crt Math) is a Windows system error that appears in the Dll category. The full message usually reads:

> "Error api-ms-win-crt-math: Api Ms Win Crt Math."
...
Most DLL errors related to Api Ms Win Crt Math are resolved by reinstalling the Visual C++ runtimes:
...
```cmd
regsvr32 /u api_ms_win_crt_math.dll
regsvr32 api_ms_win_crt_math.dll
```
