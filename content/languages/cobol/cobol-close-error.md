---
title: "[Solution] COBOL CLOSE Statement Error - How to Fix"
description: "Fix COBOL CLOSE statement errors including missing CLOSE, wrong options, and file handle issues after CLOSE."
languages: ["cobol"]
...
- Every OPEN must have a corresponding CLOSE. Missing CLOSE leaves files locked and resources unreleased, causing problems for subsequent job steps or programs.
- CLOSE WITH LOCK prevents the file from being reopened in the same run unit. Using this option incorrectly blocks access to shared datasets.
- CLOSE REEL releases the reel or volume but keeps the file position. CLOSE UNIT releases all resources. Choosing the wrong option causes unexpected file behavior.
- After CLOSE, any subsequent I/O on the file triggers an S0C4 or file status error. Code that references a closed file is undefined.
...
1. **Missing CLOSE - file locked after program ends**
2. **CLOSE WITH LOCK - file cannot be reopened**
3. **CLOSE on already-closed file - runtime error**
4. **CLOSE with wrong disposition - dataset deleted**
...
```cobol
       CLOSE IN-FILE WITH LOCK.
       *> Now IN-FILE cannot be opened again
       *> in this run unit

; CORRECT: Only use WITH LOCK when needed
       CLOSE IN-FILE.
```
...
```cobol
       CLOSE IN-FILE.
       CLOSE IN-FILE.
       *> S0C4 or status error - already closed

; CORRECT: Check before closing
       IF FILE-IS-OPEN
           CLOSE IN-FILE
           SET FILE-IS-OPEN TO FALSE.
```
...
```cobol
       CLOSE OUT-FILE.
       *> If DISP was (NEW,CATLG), dataset is cataloged
       *> If DISP was (NEW,DELETE), dataset is deleted

; CORRECT: Ensure correct DISP in JCL
       //OUTFILE DD DSN=MY.OUTPUT,DISP=(NEW,CATLG)
```
...
```cobol
       ...
       CLOSE IN-FILE.
       *> REWRITE after CLOSE would fail
```
...
```cobol
       ...
       READ IN-FILE.
       CLOSE IN-FILE.
       *> File is now closed and released
```
...
**Missing CLOSE after OPEN**

The program opens a file but never closes it. Resources remain allocated and the file may stay locked, preventing other programs from accessing it.
...
**CLOSE WITH LOCK too aggressive**

WITH LOCK is used when only a simple CLOSE is needed. This prevents the file from being reopened within the same run unit.
...
**CLOSE on file that was never opened**

The program attempts to CLOSE a file that was never successfully opened, causing a runtime error.
...
1. **Always match every OPEN with a corresponding CLOSE in the same program path.**
2. **Use CLOSE (without WITH LOCK) unless you specifically need to prevent reopening.**
3. **Set a FILE-IS-OPEN flag and check it before OPEN and CLOSE operations.**
...
- [FILE STATUS Error](/languages/cobol/cobol-file-status-error) — file I/O status codes
- [VSAM Error](/languages/cobol/cobol-vsam-error) — VSAM access issues
- [JCL Error](/languages/cobol/cobol-jcl-error) — job control errors
