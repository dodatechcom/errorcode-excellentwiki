---
title: "[Solution] COBOL DIVIDE Error - How to Fix"
description: "Fix COBOL DIVIDE statement errors including division by zero, remainder handling, and size overflow issues."
languages: ["cobol"]
...
- Division by zero is undefined in COBOL. The DIVIDE BY ZERO or DIVIDE INTO ZERO clause causes an S0C8 abend at runtime.
- DIVIDE REMAINDER stores the remainder in a specified field. If the REMAINDER field is too small, the remainder is truncated.
- The GIVING clause stores the quotient. If the GIVING field PIC is too small for the result, overflow occurs.
- DIVIDE A BY B GIVING C REMAINDER D computes C = A/B and D = A - (B * C). All three fields must be sized correctly.
...
1. **S0C8 - Division by zero exception**
2. **DIVIDE REMAINDER overflow - field too small**
3. **DIVIDE GIVING result exceeds PIC clause**
4. **DIVIDE INTO with integer division truncation**
...
```cobol
       DIVIDE WS-A BY WS-B
           GIVING WS-C
           REMAINDER WS-D.
       *> If WS-D PIC 9(2) but remainder is 100+

; CORRECT: Size REMAINDER field appropriately
       01 WS-D PIC 9(5).
       DIVIDE WS-A BY WS-B
           GIVING WS-C
           REMAINDER WS-D.
...
```cobol
       DIVIDE 100000 BY 3 GIVING WS-C.
       *> If WS-C PIC 9(4), overflow

; CORRECT: Use sufficient PIC clause
       01 WS-C PIC 9(10).
       DIVIDE 100000 BY 3 GIVING WS-C.
```
...
```cobol
       ...
       *> WS-B = WS-B / WS-A, stored in WS-B
       *> If WS-B too small, truncation

; CORRECT: Use GIVING to separate quotient
...
**Division by zero S0C8 abend**

The divisor variable contains zero. No runtime check prevents this unless explicitly coded.

**QUOTIENT overflow in DIVIDE GIVING**

The quotient exceeds the PIC clause capacity of the GIVING field. Data is truncated or an abend occurs.

**REMAINDER field too small**

The remainder from a division exceeds the PIC clause of the REMAINDER field, causing incorrect values.
...
1. **Always validate divisors are non-zero before DIVIDE statements.**
2. **Size GIVING and REMAINDER fields based on maximum possible quotient and remainder values.**
3. **Use ON SIZE ERROR exception to catch arithmetic overflow in DIVIDE operations.**
...
- [ARITHMETIC Error](/languages/cobol/cobol-arithmetic-error) — general arithmetic
- [PACKED DECIMAL Error](/languages/cobol/cobol-decimal-error) — COMP-3 issues
- [COMPUTE Error](/languages/cobol/cobol-compute-error) — expression errors
