---
title: "[Solution] COBOL SEARCH Statement Error - How to Fix"
description: "Fix COBOL SEARCH and SEARCH ALL statement errors including missing AT END, wrong INDEX usage, and search key mismatches."
languages: ["cobol"]
...
- SEARCH scans sequentially from the current INDEX position. Without AT END, an unsuccessful search falls through to the next statement.
- SEARCH ALL performs binary search. The table must be sorted on the search key. The search key must match the first field in the table entry.
- SET must initialize the INDEX before SEARCH. For SEARCH ALL, SET to 1 triggers binary search from the beginning.
- WHEN conditions in SEARCH can use compound conditions (AND, OR). Complex WHEN clauses may cause unexpected matches if logic is wrong.
...
1. **SEARCH without AT END - silent failure**
2. **SEARCH ALL on unsorted data - wrong results**
3. **INDEX not initialized before SEARCH**
4. **WHEN condition matches wrong entry**
...
```cobol
       SEARCH WS-ENTRY
           WHEN WS-ENTRY(WS-IDX) = WS-TARGET
               DISPLAY 'Found'.
       *> No AT END - falls through silently

; CORRECT: Always include AT END
       SEARCH WS-ENTRY
...
```cobol
       SEARCH ALL WS-ENTRY
           ...
           WHEN WS-KEY(WS-IDX) = WS-TARGET
               DISPLAY 'Found'.
       *> If table not sorted on WS-KEY
       *> binary search gives wrong results

; CORRECT: Ensure ascending order
```
...
```cobol
       SEARCH WS-ENTRY
           ...
           WHEN WS-ENTRY(WS-IDX) = WS-TARGET
               DISPLAY 'Found'.
       *> WS-IDX not initialized!

; CORRECT: Set before search
       SET WS-IDX TO 1.
       SEARCH WS-ENTRY ...
```
...
```cobol
       SEARCH WS-ENTRY
           AT END DISPLAY 'Not found'
           WHEN WS-ENTRY(WS-IDX) = 'A'
               DISPLAY 'Found A'
           WHEN WS-ENTRY(WS-IDX) = 'B'
               DISPLAY 'Found B'.
...
**SEARCH falls through without AT END**

An unsuccessful SEARCH does not trigger AT END (if omitted) and execution continues with the next statement, causing logic errors.

**SEARCH ALL binary search on unsorted data**

The table data is not in ascending order. The binary search may skip valid entries or return incorrect positions.

**Multiple WHEN in SEARCH first match only**

Multiple WHEN conditions match. Only the first matching WHEN is executed; subsequent matches are ignored.
...
1. **Always include AT END clause in SEARCH statements.**
2. **Sort tables in ascending order before using SEARCH ALL.**
3. **Initialize INDEX with SET before SEARCH. Use VARYING to capture the found position.**
