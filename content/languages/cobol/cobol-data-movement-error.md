---
title: "[Solution] COBOL MOVE Statement Error - How to Fix"
description: "Fix COBOL MOVE statement errors including type mismatches, truncation, and numeric MOVE unexpected behavior."
languages: ["cobol"]
...
- MOVE copies data from a sending field to a receiving field. If the receiving field is smaller, data is truncated from the right.
- NUMERIC MOVE aligns decimal points automatically. A MOVE of PIC 9(3)V99 to PIC 9(5) shifts the decimal, potentially losing fractional digits.
- ALPHA-TO-NUMERIC MOVE is undefined behavior. Moving non-numeric text to a numeric field causes S0C7 at runtime.
- MOVE of HIGH-VALUES or LOW-VALUES to numeric fields fills with invalid binary patterns, not zeros.
...
1. **MOVE truncation - receiving field too small**
2. **MOVE numeric alignment - decimal shifted unexpectedly**
3. **MOVE alpha to numeric - S0C7 abend**
4. **MOVE HIGH-VALUES to numeric - undefined**
...
```cobol
       ...
       *> If WS-LONG-NAME = 'Hello World' (11)
       *> and WS-SHORT-NAME PIC X(5)
       *> Result: 'Hello' (truncated)

; CORRECT: Ensure receiving field is large enough
       01 WS-SHORT-NAME PIC X(11).
```

### Solution 2

```cobol
       MOVE 123.45 TO WS-FIELD.
       *> If WS-FIELD PIC 9(5) (no V99)
       *> Result: 00123 (decimal lost)

; CORRECT: Use matching decimal places
       01 WS-FIELD PIC 9(5)V99.
       MOVE 123.45 TO WS-FIELD.
```
...
```cobol
       MOVE 'ABC' TO WS-NUM.
       *> WS-NUM is PIC 9(5) -> S0C7

; CORRECT: Validate before moving
       IF WS-TEXT NUMERIC
           MOVE WS-TEXT TO WS-NUM.
```
...
```cobol
       ...
       *> Fills with hex 00, which is zero
       MOVE HIGH-VALUES TO WS-NUM.
       *> Fills with hex FF, not a number
...
```cobol
       MOVE WS-A TO WS-B.
       *> If WS-A PIC 9(3) VALUE 456
       *> and WS-B PIC 9(5) VALUE 00000
       *> Result: WS-B = 00456
```
...
**MOVE truncation silently drops data**

The receiving field is smaller than the sending field. Data beyond the receiving field length is lost without warning.

**NUMERIC MOVE shifts decimal point**

A MOVE between numeric fields with different decimal places shifts the decimal point, potentially losing precision.

**MOVE text to numeric field**

Moving alphabetic or alphanumeric data to a numeric field causes S0C7 at runtime.
...
1. **Size receiving fields to accommodate maximum sending field length.**
2. **Use SAME AS to inherit field sizes from copybooks or other data definitions.**
3. **Always validate data types before MOVE operations between alphanumeric and numeric fields.**
