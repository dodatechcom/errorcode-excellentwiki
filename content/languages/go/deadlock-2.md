...
A deadlock occurs when all goroutines in a program are blocked and none can make progress. The Go runtime detects this and panics.

## Description

Go's runtime has a built-in deadlock detector. When every goroutine is blocked (on channel operations, locks, or system calls), the runtime assumes this is unintended and panics with `fatal error: all goroutines are asleep - deadlock!`. This is a development aid — production programs should avoid reaching this state.
...
// Correct — both sides run in goroutines
ch1 := make(chan int)
ch2 := make(chan int)
...
### Fix 5: Use context with timeout
