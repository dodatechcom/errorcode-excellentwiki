---
title: "[Solution] Objective-C performSelector Error - How to Fix"
description: "Fix Objective-C performSelector errors including memory management warnings, unknown selector warnings, and performSelector with objects."
...
- performSelector with ARC generates a warning because ARC cannot determine the memory management semantics of the returned value. Use objc_msgSend or suppress with pragma.
- performSelector withObject passes an object as the argument. If the selector expects a different type, the behavior is undefined.
- performSelector with a nil target or unknown selector returns nil/0 silently. This can mask bugs where the target should not be nil.
- The return type of performSelector is always id. Cast the result to the expected type when the selector returns a specific type.
...
1. **ARC memory management warning with performSelector**
2. **performSelector unknown selector warning**
3. **Return type mismatch from performSelector**
4. **performSelector with wrong argument type**
...
[obj performSelector:@selector(selector) withObject:nil];
...
```objective-c
NSString *result = [obj performSelector:@selector(name)];
// Warning: incompatible pointer to integer
// CORRECT:
NSString *result = (NSString *)[obj performSelector:@selector(name)];
```
...
```objective-c
[obj performSelector:@selector(setValue:) withObject:value];
// Value must be an object (id)
// Primitive types need boxing
```
...
```objective-c
// Use NSInvocation for complex calls
NSInvocation *inv = [NSInvocation invocationWithMethodSignature:
    [obj methodSignatureForSelector:@selector(doWork:)]];
[inv setTarget:obj];
...
**ARC memory management warning**

ARC cannot determine if performSelector returns an owned or unretained reference. This causes a -Warc-performSelector-leaks warning.
...
**Unknown selector at runtime**

The selector name does not match any method on the target object. The call fails silently or crashes depending on the context.
...
**Return type cast required**

performSelector returns id. If the actual method returns an int or other type, the result must be cast appropriately.
...
1. **Use #pragma to suppress performSelector warnings when the memory semantics are understood.**
2. **Always check respondsToSelector before performSelector to catch unknown selectors early.**
3. **Use NSInvocation for complex invocations that require argument boxing or return value handling.**
...
- [RUNTIME Error](/languages/objective-c/exc-bad-access) — runtime issues
