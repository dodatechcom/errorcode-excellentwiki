---
title: "[Solution] Objective-C Core Data Error - How to Fix"
description: "Fix Objective-C Core Data errors including context save failures, migration errors, and relationship integrity violations."
...
- NSManagedObjectContext is not thread-safe. Accessing managed objects from the wrong thread causes illegal relationship or similar errors.
- save returns a boolean and may populate the error parameter. Ignoring the error parameter means save failures go undetected.
- NSPersistentStore migration errors occur when the data model changes without a mapping model. Lightweight migration must be enabled for automatic schema updates.
- Relationship integrity violations happen when a required relationship is nil or when deleting objects with mandatory inverse relationships.
...
1. **Core Data context used on wrong thread**
2. **save failure ignored - error parameter nil**
3. **Persistent store migration failed**
4. **Required relationship is nil**
...
dispatch_async(
    dispatch_get_global_queue(...), ^{
        // WRONG: Use context on background
        NSManagedObject *obj = ...;
        // Must use performBlock:
        [context performBlock:^{
            // Safe context access here
        }];
    });
```
...
**Context used on wrong thread**

A managed object context is accessed from a thread other than the one it was created on. Core Data contexts are not thread-safe.
...
The save method returns NO but the error parameter is not checked. The save silently fails and data is lost.
...
The data model changed but lightweight migration options were not provided to the persistent store coordinator.
...
1. **Use performBlock for all Core Data operations on a context to ensure thread safety.**
2. **Always check the return value and error parameter of save and rollback when errors occur.**
3. **Enable lightweight migration options when adding new model versions.**
