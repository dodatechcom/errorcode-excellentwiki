---
title: "[Solution] Objective-C Instance Variable Error - How to Fix"
description: "Fix Objective-C instance variable errors including direct ivar access, ARC ivar management, and memory layout issues in subclasses."
...
- Directly accessing instance variables (ivar) bypasses property setters and getters, skipping KVO notifications and custom access logic. This can cause subtle bugs.
- In ARC, ivars are implicitly strong. Declaring an ivar as __strong creates an implicit retain that may cause unexpected retain cycles.
- Subclass ivars are appended to the superclass ivar layout. If the superclass changes its ivar layout, subclass binary compatibility may break.
- Ivars declared in categories are not supported. Attempting to add ivars in a category causes a compiler error. Use associated objects instead.
...
1. **Direct ivar access bypasses KVO**
2. **Implicit strong ivar causing retain cycle**
3. **Subclass ivar layout change breaks binary**
4. **Cannot declare ivar in category**
...
- (void)updateName {
    _name = @"New"; // Bypasses setter!
    // KVO not triggered
}

// CORRECT: Use self.name = @"New"
@end
```
...
```objective-c
@interface MyClass : NSObject {
    __strong id _data; // Implicit strong
    __weak id _delegate; // Weak ivar
}
@end
```
...
@interface SubClass : SuperClass {
    id _ivar2; // Added after _ivar1
}
@end
// Changing SuperClass ivars affects SubClass layout
```
...
// CORRECT: Use associated objects
objc_setAssociatedObject(self, &key, @(count), ...);
```
...
Code directly assigns to _name ivar instead of using self.name. KVO observers registered on the name property are not notified of the change.
...
**Retain cycle from strong ivar**

A strong ivar holds a reference to an object that also references the owner. The implicit strong ivar creates a retain cycle.
...
**Category ivar compiler error**

A category tries to add an instance variable. The compiler rejects this because categories cannot modify the instance variable layout of a class.
...
1. **Use property access (self.property) instead of direct ivar access (_ivar) to ensure KVO compliance and custom logic.**
2. **Declare delegate ivars as __weak to avoid retain cycles in Manual Reference Counting.**
3. **Use associated objects instead of ivars in categories. Use class extensions for additional ivars.**
...
- [KVO Error](/languages/objective-c/objc-kvo-error) — KVO issues
- [MEMORY Error](/languages/objective-c/objc-memory-error) — memory issues
- [CATEGORY Error](/languages/objective-c/objc-category-error) — category issues
