---
title: "[Solution] Objective-C File Not Found Error - How to Fix"
description: "Fix Objective-C file not found errors when loading resources, bundles, or files using incorrect paths or missing NSBundle configuration."
...
- NSBundle mainBundle path for resources depends on the bundle being correctly configured. Resources not added to the Xcode target are not included in the bundle and cause file not found errors.
- File paths constructed with string concatenation may fail on different systems due to path separators, sandbox restrictions, or different directory structures.
- Case sensitivity matters on case-sensitive file systems (iOS device, Linux). A file referenced as MyImage.png when the actual file is myimage.png causes a file not found error.
- Documents directory and other sandbox paths may not exist at runtime. The app must create them before attempting to write files.
...
1. **NSBundle pathForResource returns nil**
2. **File not found at constructed path**
3. **Case-sensitive file lookup failed**
4. **Documents directory does not exist**
...
if (!path) {
    ...
    // Check that file is in Copy Bundle Resources
}
```
...
// Case-sensitive: myimage.png != MyImage.png
...
if (![fm fileExistsAtPath:docDir]) {
    [fm createDirectoryAtPath:docDir
...
**Resource not in bundle**

An image file exists in the project but was not added to the Copy Bundle Resources build phase. pathForResource returns nil at runtime.
...
The code references MyImage.png but the actual file is myimage.png. On case-sensitive file systems, this causes a file not found error.
...
**Sandbox directory missing**

The app tries to write to the Documents directory but it has not been created yet. The write operation fails with a file not found error.
...
1. **Verify that all resources are in the Copy Bundle Resources build phase in Xcode.**
2. **Match exact file names and extensions when using pathForResource:ofType:.**
3. **Always check if a directory exists before writing files. Create directories with createDirectoryAtPath:when needed.**
...
- [I/O Error](/languages/objective-c/objc-io-error) — I/O operations
