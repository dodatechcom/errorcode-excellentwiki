---
title: "[Solution] Objective-C Weak Reference Error - How to Fix"
description: "Fix Objective-C weak reference errors when weak references become nil unexpectedly or cause undefined behavior in non-ARC code."
...
- Weak references are automatically set to nil when the referenced object is deallocated. Code that uses a weak reference after deallocation sees nil, which may not be expected.
- In non-ARC code (MRC), weak references are not auto-zeroing. The pointer becomes dangling and accessing it causes EXC_BAD_ACCESS.
- __unsafe_unretained is stronger than __weak - it does not auto-zero. The pointer remains pointing to deallocated memory, causing undefined behavior.
- Weak references in IBOutlet connections may become nil when the view hierarchy is torn down, causing unexpected nil dereferences in view lifecycle methods.
...
1. **Weak reference became nil unexpectedly**
2. **Dangling pointer from __unsafe_unretained**
3. **IBOutlet weak reference nil in dealloc**
4. **Weak reference used after deallocation**
...
// After delegate object dealloc:
// self.delegate becomes nil automatically
...
```objective-c
// __unsafe_unretained does NOT auto-zero
__unsafe_unretained id unsafeRef = someObject;
[someObject release]; // In MRC
// unsafeRef is now dangling!

// CORRECT: Use __weak (ARC only)
__weak id weakRef = someObject;
```
...
```objective-c
// IBOutlet - set to nil in dealloc
- (void)dealloc {
    _myLabel = nil;
    _myButton = nil;
}
...
```objective-c
// Check nil before using weak ref
...
The delegate object is deallocated. The weak reference auto-zeros. Code that does not check for nil before calling delegate methods silently skips the call.
...
**Weak IBOutlet nil after unload**

The view is unloaded due to memory pressure. IBOutlet weak references become nil. Accessing them in viewDidLoad after reload works, but accessing them before reload returns nil.
...
**MRC dangling pointer**

In Manual Reference Counting, __unsafe_unretained references do not auto-zero. The pointer remains pointing to freed memory, causing crashes or data corruption.
...
1. **Always check weak references for nil before using them, especially in completion blocks and delegate callbacks.**
2. **Use __weak (ARC) instead of __unsafe_unretained to benefit from auto-zeroing behavior.**
3. **Set IBOutlet properties to nil in dealloc to ensure clean teardown of view hierarchy references.**
...
- [STRONG REFERENCE CYCLE](/languages/objective-c/objc-strong-reference-cycle) — retain cycles
- [MEMORY Error](/languages/objective-c/objc-memory-error) — memory issues
- [RUNTIME Error](/languages/objective-c/exc-bad-access) — access violations
