---
title: "[Solution] Objective-C Thread Safety Error - How to Fix"
description: "Fix Objective-C threading errors including race conditions, UIKit updates from background threads, and GCD synchronization issues."
...
- UIKit is not thread-safe. All UI updates must happen on the main thread. Updating UI from a background thread causes rendering glitches, crashes, or undefined behavior.
- Shared mutable state accessed from multiple threads without synchronization causes race conditions. The outcome depends on unpredictable thread scheduling.
- NSLock, @synchronized, and dispatch_sync provide different levels of synchronization. Using the wrong primitive may not prevent the race condition.
- dispatch_once is the correct way to perform thread-safe one-time initialization. Using static variables without dispatch_once is not thread-safe.
...
1. **UIKit update from background thread**
2. **Race condition on shared mutable state**
3. **Thread-unsafe singleton initialization**
4. **Deadlock from nested synchronization**
...
@synchronized(self.sharedData) {
    self.sharedData.count++;
}
```
...
@synchronized(lockA) {
    @synchronized(lockB) {
        // If another thread holds lockB
        // and waits for lockA = deadlock
    }
}
...
**UIKit update from background thread**

A network callback arrives on a background thread and updates a UILabel. This causes undefined behavior or EXC_BAD_ACCESS.
...
**Race condition in property access**

Two threads simultaneously increment a shared counter. Without synchronization, some increments are lost (read-modify-write race).
...
A singleton is initialized with a simple nil check. Two threads may both see nil and create two instances.
...
1. **Always dispatch UI updates to the main queue with dispatch_async(dispatch_get_main_queue(), ^{ ... }).**
2. **Use @synchronized, NSLock, or serial dispatch queues to protect shared mutable state.**
3. **Use dispatch_once for thread-safe one-time initialization of singletons and caches.**
...
- [GCD Error](/languages/objective-c/objc-gcd-error) — dispatch issues
