---
title: "[Solution] Objective-C EXC_BAD_ACCESS Error - How to Fix"
description: "Fix Objective-C EXC_BAD_ACCESS errors when accessing deallocated memory, including dangling pointers and use-after-free scenarios."
...
- EXC_BAD_ACCESS means the program tried to access memory that is no longer valid. This is the most common Objective-C runtime crash and is caused by accessing freed objects.
- Dangling pointers occur when a pointer references an object that has been freed. The pointer becomes invalid but is still used, causing EXC_BAD_ACCESS.
- Thread-unsafe access to objects that are being deallocated on another thread causes unpredictable crashes. The deallocation timing depends on thread scheduling.
- Stack buffer overflows can corrupt adjacent memory, including object pointers, causing later EXC_BAD_ACCESS when the corrupted pointer is dereferenced.
...
2. **Thread 1: EXC_BAD_ACCESS at address 0x...**
3. **Signal: SIGSEGV (Segmentation fault)**
4. **Zombie object message sent to deallocated instance**
...
// This detects use-after-free, buffer overflows, etc.
```
...
```objective-c
// Thread-safe access with dispatch_sync
...
```objective-c
// Use Instruments > Zombies to track
// deallocated object access patterns
// The stack trace shows exactly where
// the deallocated object was accessed
```
...
**EXC_BAD_ACCESS after dismiss**

A presented view controller is dismissed. A retained block still holds a strong reference to it. When the block executes, it accesses the deallocated controller.
...
A background operation accesses an object that was deallocated on the main thread. The crash occurs on the background thread but the root cause is the deallocation timing.
...
**EXC_BAD_ACCESS in callback**

A network callback arrives after the requesting object has been freed. The callback tries to deliver results to the deallocated object.
...
1. **Enable Address Sanitizer in Xcode for comprehensive memory error detection during development.**
2. **Use Instruments Zombies template to identify the exact object and access point causing EXC_BAD_ACCESS.**
3. **Convert strong references to weak for delegates, observers, and back-references to prevent retain cycles.**
