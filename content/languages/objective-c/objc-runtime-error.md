---
title: "[Solution] Objective-C Runtime Error - How to Fix"
description: "Fix Objective-C runtime errors including objc_msgSend failures, method resolution failures, and dynamic class creation issues."
...
- objc_msgSend is the core of Objective-C messaging. When the receiver is nil, objc_msgSend returns 0. When the method is not found, the runtime attempts dynamic resolution before crashing.
- Dynamic class registration with objc_allocateClassPair requires the class name to be unique. Registering a duplicate class name fails and returns nil.
- The runtime uses isa swizzling for method caching. If the isa pointer is corrupted, method dispatch fails with EXC_BAD_ACCESS.
- Forwarding an unhandled message requires implementing forwardInvocation: or methodSignatureForSelector:. Missing these methods causes the runtime to raise an unrecognized selector exception.
...
1. **objc_msgSend failure - bad receiver**
2. **Dynamic class registration failed**
3. **isa pointer corrupted - EXC_BAD_ACCESS**
4. **Message forwarding failed - no methodSignature**
...
```objective-c
id obj = nil;
[obj doWork]; // Safe: returns nil

// But primitives cause issues:
int result = [nil doWork]; // result = 0
// Not always the expected default
```
...
Class cls = objc_allocateClassPair(
    [NSObject class], "MyDynamicClass", 0);
if (cls == nil) {
...
```objective-c
id obj = ...; // obj with corrupted isa
[obj doWork]; // EXC_BAD_ACCESS

// CORRECT: Validate object before use
if (obj && [obj respondsToSelector:@selector(doWork)]) {
    [obj doWork];
}
```
...
- (void)forwardInvocation:(NSInvocation *)inv {
    [inv setTarget:self.defaultTarget];
    [inv invoke];
}

- (NSMethodSignature *)methodSignatureForSelector:(SEL)aSelector {
    return [self.defaultTarget methodSignatureForSelector:aSelector];
}
```
...
**objc_msgSend with nil receiver**

Sending a message to nil is safe in Objective-C but may produce unexpected zero/nil return values that propagate through the application.
...
**Dynamic class name collision**

Two parts of the application try to register the same dynamic class name. The second registration fails silently.
...
**Message forwarding incomplete**

The class implements forwardInvocation but not methodSignatureForSelector, causing the forwarding mechanism to fail.
...
1. **Always check for nil before using return values from messages sent to potentially nil objects.**
2. **Use unique class name prefixes for dynamically registered classes to avoid name collisions.**
3. **Implement both methodSignatureForSelector and forwardInvocation for complete message forwarding.**
...
- [METHOD Error](/languages/objective-c/objc-method-error) — method issues
