---
title: "[Solution] Objective-C NSNotificationCenter Error - How to Fix"
description: "Fix Objective-C NSNotificationCenter errors including notification observer leaks, posting to wrong notification center, and thread-safety issues."
...
- Forgetting to remove an observer causes a crash when the notification is posted after the observer is deallocated. Use removeObserver in dealloc.
- Posting a notification on a background thread delivers it on that thread. UI updates in the notification handler must dispatch to the main thread.
- Using defaultCenter when you should use a custom notification center causes notifications to be delivered to the wrong observers.
- Notification selectors must match the expected signature: (void)handler:(NSNotification *)notification. Wrong signature causes undefined behavior.
...
1. **Crash posting notification after observer deallocated**
2. **Notification delivered on background thread**
3. **Wrong notification center used**
4. **Notification handler signature mismatch**
...
The observer object is deallocated while still registered for notifications. When the notification fires, the runtime attempts to message a deallocated object.
...
**Notification on background thread**

A notification posted on a background thread delivers its callbacks on the posting thread. UI updates from the handler will crash or cause undefined behavior.
...
**Wrong notification center**

The notification is posted to one center but the observer registered with another center.
...
1. **Always remove notification observers in dealloc or viewDidDisappear.**
2. **Check if on the main thread in notification handlers and dispatch to main thread if needed.**
3. **Use a consistent notification center throughout your application.**
