---
title: "[Solution] Objective-C Property Attribute Error - How to Fix"
description: "Fix Objective-C property attribute errors when using wrong memory management, atomicity, or accessor attributes on properties."
...
- Using strong when weak is needed creates retain cycles. Using weak when the object lifetime is not guaranteed causes nil references at unexpected times.
- nonatomic properties are faster but not thread-safe. Two threads writing to a nonatomic property simultaneously may cause data corruption or crashes.
- readonly properties prevent external modification. Removing readonly unexpectedly exposes the property to external modification, breaking encapsulation.
- copy semantics create a new copy of the assigned value. For mutable objects, copy may return an immutable version, changing the expected behavior.
...
1. **Retain cycle from wrong property attribute**
2. **Thread-unsafe nonatomic property access**
3. **Property not settable - readonly violation**
4. **Copy returns immutable type**
...
// Strong = retain cycle risk

// CORRECT for delegate:
@property (nonatomic, weak) id<MyDelegate> delegate;

// CORRECT for owner reference:
...
// nonatomic is fine for single-threaded access
```
...
// External code cannot modify identifier
// model.identifier = @"new"; // Compiler error
```
...
// copy returns NSArray, not NSMutableArray!

// CORRECT:
@property (nonatomic, strong) NSMutableArray *items;
// Or copy with custom setter
```
...
**Retain cycle from strong delegate**

A delegate property is declared as strong instead of weak. The delegate also references the owner, creating a retain cycle that leaks both objects.
...
**nonatomic race condition**

Two threads simultaneously set a nonatomic string property. One thread reads a partially-written pointer value, causing a crash.
...
**Copy on NSMutableArray**

A property declared as copy receives an NSMutableArray. The copy method returns an immutable NSArray, causing crashes when mutable methods are called on it.
...
1. **Use weak for delegate and back-reference properties. Use strong for owned objects.**
2. **Default to nonatomic unless the property is accessed from multiple threads.**
3. **Use copy for NSString and NSArray to protect against mutable subclass mutations. Use strong for NSMutableArray.**
...
- [THREAD Error](/languages/objective-c/objc-thread-error) — threading
