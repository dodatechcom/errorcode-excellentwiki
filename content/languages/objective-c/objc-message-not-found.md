---
title: "[Solution] Objective-C Unrecognized Selector Error - How to Fix"
description: "Fix Objective-C unrecognized selector sent to instance errors when calling methods that don't exist on an object."
...
- The 'unrecognized selector sent to instance' error occurs when you call a method that the object class does not implement. The runtime cannot find the method implementation.
- Dynamic method resolution allows the runtime to resolve selectors at runtime. If the method is defined in a category that was not loaded, the resolution fails.
- Methods defined in protocols must be implemented by conforming classes. Calling an optional protocol method that is not implemented raises this error.
- Typo in method name is the most common cause. The compiler may not catch it if the method is called via performSelector or id type.
...
2. **Method not found in class or categories**
3. **Optional protocol method not implemented**
4. **Dynamic method resolution failed**
...
// Crashes: unrecognized selector

// CORRECT: Check respondsToSelector
...
// Runtime cannot find implementation

// CORRECT: Implement method or use NSInvocation
```
...
// Calling optionalMethod on non-implementer crashes
...
// NSString does not have this method!

// CORRECT: Use NSAttributedString or UILabel
```
...
A method is called with a misspelled name. The compiler may not catch this if the receiver type is id or if the method is called dynamically.
...
**Category method not loaded**

A method defined in a category is not available because the category implementation file was not included in the build.
...
**Optional protocol method not implemented**

An optional protocol method is called on an object that does not implement it without checking respondsToSelector first.
...
1. **Always use respondsToSelector before calling methods on id-typed objects.**
2. **Ensure category implementation files are included in the build target.**
3. **Implement all @required protocol methods and check @optional methods with respondsToSelector before calling.**
...
- [RUNTIME Error](/languages/objective-c/exc-bad-access) — memory issues
