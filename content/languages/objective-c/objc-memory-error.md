---
title: "[Solution] Objective-C Memory Error - How to Fix"
description: "Fix Objective-C memory errors including EXC_BAD_ACCESS, use-after-free, and excessive memory consumption from unchecked allocations."
...
- EXC_BAD_ACCESS occurs when accessing memory that has been freed. Under ARC, this usually indicates a strong reference cycle that prevented deallocation, followed by manual access.
- Zombie objects in Xcode help detect use-after-free. The deallocated object is replaced with a zombie that logs the access attempt before crashing.
- Excessive memory consumption from images, data buffers, or cached objects can cause memory warnings and eventual termination by the OS.
- Under ARC, objects are released when their reference count reaches zero. Cycles prevent this from happening, keeping both objects alive indefinitely.
...
1. **EXC_BAD_ACCESS - memory access violation**
2. **Use-after-free detected by zombie objects**
3. **Application received memory warning**
4. **Terminated due to memory pressure**
...
// This replaces freed objects with NSZombie
// that logs access before crashing
```
...
// Allocations instrument tracks all heap allocations
// Leaks instrument detects reference cycles
// Use Time Profiler to find hot paths
```
...
**EXC_BAD_ACCESS after view controller dismissal**

A view controller is dismissed but a retained block or notification still references it. The next access to the deallocated object crashes.
...
**Memory warning from image caching**

An image cache accumulates large UIImage objects without eviction. The system sends a memory warning and eventually terminates the app.
...
**Use-after-free in delegate callback**

A delegate callback fires after the delegate object is deallocated. The zombie log shows the exact access point.
...
1. **Enable zombie objects during development to quickly identify use-after-free issues.**
2. **Use weak references for delegates, blocks, and notification observers to prevent retain cycles.**
3. **Monitor memory usage with Instruments and implement memory warning handlers to release cached resources.**
...
- [BLOCK Error](/languages/objective-c/objc-block-error) — block issues
