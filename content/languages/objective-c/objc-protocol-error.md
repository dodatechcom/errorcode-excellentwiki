---
title: "[Solution] Objective-C Protocol Conformance Error - How to Fix"
description: "Fix Objective-C protocol conformance errors when classes fail to implement required protocol methods or declare protocol conformance incorrectly."
...
- A class that declares conformance to a protocol must implement all @required methods. Missing required methods cause compiler warnings in debug builds and runtime crashes in production.
- Protocol conformance is checked at compile time for static types but at runtime for id-typed objects. Dynamic protocol checks require conformsToProtocol or respondsToSelector.
- A protocol can inherit from other protocols. The implementing class must satisfy all inherited @required methods, not just those declared in the primary protocol.
- @optional protocol methods do not need to be implemented, but calling them without checking respondsToSelector causes crashes.
...
1. **Missing @required protocol method implementation**
2. **Protocol inheritance - inherited methods not implemented**
3. **@optional method called without check**
4. **Protocol declared but not adopted**
...
// Must implement BOTH parentMethod and childMethod
```
...
The class does not implement all methods marked @required in the protocol. The compiler warns but the runtime crashes when the method is called.
...
**Protocol not imported**

The protocol header is not imported in the class that declares conformance. The compiler may not enforce method implementation checks.
...
**Calling optional method without check**

An @optional protocol method is called without first checking respondsToSelector. If the object does not implement the method, it crashes.
...
1. **Implement all @required methods declared in adopted protocols and their parent protocols.**
2. **Import the protocol header in any file that uses or declares conformance to the protocol.**
3. **Always check respondsToSelector before calling @optional protocol methods.**
