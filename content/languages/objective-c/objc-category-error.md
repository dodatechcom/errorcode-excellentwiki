---
title: "[Solution] Objective-C Category Error - How to Fix"
description: "Fix Objective-C category errors including name collisions, missing implementation files, and method overriding in categories."
...
- Category methods must have unique names across the entire program. If two categories define the same method, the behavior is undefined and one implementation silently replaces the other.
- Category method implementations are not compiled unless the .m file is included in the build. Missing category files cause unrecognized selector errors at runtime.
- Categories cannot add instance variables. Attempting to use @synthesize in a category causes a compiler error. Use associated objects instead.
- Overriding methods in categories is dangerous because the last-loaded category wins. The override order depends on build order, which is non-deterministic.
...
1. **Category method name collision**
2. **Category .m file not included in build**
3. **Cannot add instance variables in category**
4. **Category overriding methods unpredictably**
...
```objective-c
// Two categories with same method
@interface NSString (Cat1)
- (NSString *)helper;
@end
@interface NSString (Cat2)
- (NSString *)helper;
@end
// Undefined behavior!
```
...
// Cannot add: NSInteger _count;
// Use associated objects instead
@end

#import <objc/runtime.h>
objc_setAssociatedObject(self, &key,
    @(count), OBJC_ASSOCIATION_RETAIN);
```
...
```objective-c
// Overriding a method in a category
@implementation NSString (Override)
- (NSString *)description {
    return @'Overridden';
    // Dangerous! May break other code.
}
@end
```
...
@interface NSString (ABCPrefix_MyHelper)
- (NSString *)abc_reversedString;
@end
```
...
**Category method name collision**

Two categories on the same class define a method with the same name. The last-loaded implementation wins, causing unpredictable behavior.
...
**Category file not in build**

A category implementation file is not included in the Xcode target. The methods are declared but not implemented, causing runtime crashes.
...
**Category overrides original method**

A category redefines a method that exists in the original class. This silently replaces the original implementation, potentially breaking the class.
...
1. **Use unique method name prefixes for categories to avoid collisions.**
2. **Ensure all category .m files are included in the build target.**
3. **Never override methods in categories. Use method swizzling only as a last resort.**
