---
title: "[Solution] Objective-C Strong Reference Cycle - How to Fix"
description: "Fix Objective-C strong reference cycles where two objects hold strong references to each other, preventing deallocation."
...
- A strong reference cycle occurs when Object A holds a strong reference to Object B, and Object B holds a strong reference to Object A. Neither object can be deallocated.
- ARC (Automatic Reference Counting) cannot break strong reference cycles. Objects involved in cycles will leak memory until the application is terminated.
- Use __weak or __unsafe_unretained to break strong reference cycles in delegate patterns and block captures.
- The Instruments Leaks tool and the Memory Graph Debugger can detect strong reference cycles at development time.
...
1. **Memory leak from retain cycle**
2. **Object never deallocated - reference count never reaches zero**
3. **Block captures self strongly**
4. **Delegate holds strong reference to delegating object**
...
// If model also holds strong ref to VC:
// Retain cycle!

// CORRECT: Use weak for back-reference
...
```objective-c
// Block captures self strongly
[self doSomethingWithBlock:^{
    [self updateUI]; // Retain cycle!
}];

// CORRECT: Use weakSelf
...
```objective-c
// Block captures strong self in property
self.myBlock = ^{
...
@end
// Cell strongly holds dataSource
// DataSource strongly holds cell = cycle

// CORRECT: Use weak
@property (nonatomic, weak) MyDataSource *dataSource;
```
...
A block property stores a block that captures self with a strong reference. Since self owns the block property, a retain cycle is created.
...
**Two objects mutually retain**

Object A retains Object B and Object B retains Object A. Reference counts never reach zero.
...
**Delegate property should be weak**

A delegate property with strong creates a retain cycle if the delegate also holds a reference to the owner.
...
1. **Use __weak references in blocks that capture self. Check for nil before using.**
2. **Declare delegate properties as weak to avoid retain cycles.**
3. **Use the Memory Graph Debugger and Instruments to detect retain cycles during development.**
...
- [DELEGATE Error](/languages/objective-c/objc-delegate-error) — delegate patterns
