---
title: "[Solution] Objective-C Nil-Terminated Array Error - How to Fix"
description: "Fix Objective-C nil-terminated array errors when using C-style arrays with nil sentinels, causing out-of-bounds access or missing termination."
...
- Variadic methods like arrayWithObjects require a nil sentinel to mark the end. Forgetting the nil causes the method to read past the array into garbage memory.
- C-style arrays passed to functions expecting nil-terminated lists cause undefined behavior if they do not end with a NULL element.
- Modern Objective-C provides array and dictionary literals (@[] and @{}) which do not require nil termination. Using these eliminates the sentinel problem entirely.
- When building nil-terminated arrays dynamically, inserting nil into the middle of the array causes the variadic method to terminate early, silently dropping later elements.
...
1. **EXC_BAD_ACCESS from missing nil sentinel**
2. **Array truncated due to nil in middle**
3. **Variadic method reads past array end**
4. **C array passed without NULL termination**
...
```objective-c
// Nil in middle truncates array
NSString *middle = nil;
NSArray *arr = [NSArray arrayWithObjects:
    @"A", middle, @"C", nil];
// arr = @[@"A"] only!

// CORRECT: Filter nil values first
```
...
```objective-c
// WRONG: C array without NULL
const char *args[] = {"a", "b", "c"};
// execvp expects NULL-terminated!
...
**Variadic method crash**

arrayWithObjects is called without a nil terminator. The method reads past the provided arguments into garbage memory, causing EXC_BAD_ACCESS.
...
**Array silently truncated**

A nil value appears in the middle of the argument list. The variadic method treats nil as the terminator, dropping all subsequent elements.
...
**C array not NULL-terminated**

A C string array is passed to a function expecting NULL termination. The function reads past the array into adjacent memory.
...
1. **Use modern Objective-C literal syntax (@[], @{}) instead of variadic factory methods to avoid nil sentinel issues.**
2. **Never insert nil into a variadic argument list. Filter nil values before building the array.**
3. **Always add NULL to the end of C arrays that will be consumed as nil-terminated or NULL-terminated lists.**
