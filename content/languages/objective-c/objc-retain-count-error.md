---
title: "[Solution] Objective-C Retain Count Error - How to Fix"
description: "Fix Objective-C retain count errors when objects are retained and released incorrectly, causing premature deallocation or memory leaks."
...
- In Manual Reference Counting (MRC), every retain must have a matching release. Imbalanced retain/release causes either premature deallocation (too many releases) or memory leaks (too many retains).
- Autorelease pools manage temporary objects. Objects added to an autorelease pool are released when the pool is drained. Using objects after the pool drains causes EXC_BAD_ACCESS.
- Under ARC, retain/release are automatic. However, CF objects and __bridge casts still require manual retain count management.
- Copy semantics create a new retained copy. Assigning a mutable object to a copy property creates an independent copy, increasing the retain count of the new copy.
...
1. **Over-release - object freed too early**
2. **Under-release - memory leak**
3. **Object accessed after autorelease pool drain**
4. **CF object retain count mismatch**
...
@autoreleasepool {
    NSString *str = [NSString stringWithFormat:@"%d", 42];
    // str is in pool
}
// Pool drained - str is released
// Do NOT use str here!
```
...
```objective-c
// CF object requires manual management
CFStringRef cfStr = CFStringCreateWithCString(...);
// CF objects are NOT managed by ARC
CFRelease(cfStr); // Must release manually
```
...
**Premature deallocation in MRC**

An object is released one too many times. The next access to the freed memory causes EXC_BAD_ACCESS or data corruption.
...
**Autorelease pool exhaustion**

A loop creates thousands of autoreleased objects without an inner @autoreleasepool. Memory grows until the system kills the process.
...
A Core Foundation object is created but never released. The memory is leaked until the process terminates.
...
1. **Use ARC instead of MRC to eliminate manual retain/release management.**
2. **Use inner @autoreleasepool blocks in loops that create many temporary objects.**
3. **Release Core Foundation objects with CFRelease or bridge them to ARC-managed types.**
...
- [ARC Error](/languages/objective-c/objc-arc-error) — ARC issues
