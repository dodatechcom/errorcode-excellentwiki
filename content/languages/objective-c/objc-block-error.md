---
title: "[Solution] Objective-C Block Capture Error - How to Fix"
description: "Fix Objective-C block capture errors when blocks incorrectly capture variables, causing retain cycles, undefined behavior, or unexpected values."
...
- Blocks capture __block variables by reference and all other variables by value (copy). A __block variable changes after the block is created may not be visible inside the block.
- Capturing a self-referencing object in a block creates a retain cycle unless __weak is used. The block retains self, and self retains the block property.
- Stack-allocated blocks copied to the heap via Block_copy may capture dangling pointers if the original variables go out of scope.
- Variables captured by blocks in ARC are implicitly __strong. This means blocks retain captured objects, which can create retain cycles.
...
1. **__block variable not captured correctly**
2. **Block retains self causing retain cycle**
3. **Dangling pointer from stack block capture**
4. **Implicit strong capture creates leak**
...
**Variable captured by value not by reference**

Non-__block variables are captured by value. Changes to the variable after block creation are not visible inside the block.
...
**__block variable retain cycle**

__block variables that capture objects may create retain cycles if the block is stored and the object also owns the block.
...
**Stack block used after scope**

A block defined on the stack is used after the scope where it was defined has exited, causing undefined behavior.
...
1. **Use __block to allow variable mutation inside blocks. Be aware of retain semantics.**
2. **Always use __weak when blocks need to reference self. Check for nil and use __strong inside the block.**
3. **Copy stack blocks to the heap with [block copy] if they need to outlive their defining scope.**
