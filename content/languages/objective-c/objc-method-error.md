---
title: "[Solution] Objective-C Method Signature Error - How to Fix"
description: "Fix Objective-C method signature errors including wrong return types, missing method declarations, and Objective-C/C interop calling convention issues."
...
- A method declared in the header must have a matching implementation in the .m file. A mismatch in return type, parameter count, or parameter types causes undefined behavior.
- Objective-C methods use hidden id and SEL parameters (self and _cmd). C function calls do not. Mixing these calling conventions causes stack corruption.
- Method signatures in protocols must exactly match the implementing class. Even minor differences in parameter types cause the compiler to not recognize the implementation.
- Overriding a method with a different signature than the superclass causes the override to not be recognized by the runtime. The superclass method is called instead.
...
1. **Method declaration does not match definition**
2. **Calling convention mismatch**
3. **Protocol method signature mismatch**
4. **Override signature differs from superclass**
...
```objective-c
// WRONG: C function for ObjC method
void myMethod(id self, SEL _cmd, NSString *param);
// Stack corruption!
...
- (void)process:(NSInteger)count;
@end

@interface Child : Parent
- (void)process:(NSNumber *)count; // Different type!
@end
// Override not recognized at runtime
```
...
**Header and implementation mismatch**

The header declares a method returning NSString but the implementation returns NSData. The compiler may not catch this if the types are compatible, but runtime behavior is undefined.
...
**Protocol method signature wrong**

A protocol declares -(BOOL)validate:(id)input but the implementing class declares -(BOOL)validate:(NSString *)input. The compiler does not recognize the implementation.
...
**Override with wrong parameter type**

A subclass overrides a method with a different parameter type. The runtime dispatches to the superclass method, not the subclass override.
...
1. **Ensure method signatures in headers exactly match their implementations, including return types and parameter types.**
2. **When overriding superclass methods, use the exact same signature. Use the override keyword to let the compiler verify.**
3. **Match protocol method signatures exactly. Even adding or removing const can cause mismatches.**
...
- [PROTOCOL Error](/languages/objective-c/objc-protocol-error) — protocol issues
