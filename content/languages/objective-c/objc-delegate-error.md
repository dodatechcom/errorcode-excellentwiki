---
title: "[Solution] Objective-C Delegate Error - How to Fix"
description: "Fix Objective-C delegate errors including nil delegate calls, delegate protocol conformance issues, and retain cycles in delegate patterns."
...
- Calling a delegate method on a nil delegate is safe in Objective-C (message to nil returns nil/0). However, if the delegate is expected to handle the message, important logic may be skipped.
- A delegate must conform to the delegate protocol declared in the property. If the delegate does not conform, calling methods not in the protocol may crash.
- Using strong delegate property creates a retain cycle. The delegate should always be declared as weak to avoid this common memory leak.
- Optional delegate methods should be checked with respondsToSelector before calling. Required methods are assumed to be implemented.
...
1. **Nil delegate - delegate method not called**
2. **Delegate does not conform to protocol**
3. **Strong delegate creates retain cycle**
4. **Optional protocol method not checked**
...
// If delegate is nil:
[delegate didUpdate]; // Safe: no-op
// But logic is skipped!

// CORRECT: Ensure delegate is set
```
...
// If dataSource is not UITableViewDataSource:
// Crashes at runtime

// CORRECT: Use proper protocol
...
// Without check:
[delegate didFinish]; // May crash if not implemented
...
**Nil delegate silently drops messages**

When the delegate is nil, messages are silently ignored. This can cause missing functionality without any error indication.
...
**Delegate does not implement required methods**

The delegate object does not implement all @required protocol methods, causing crashes when those methods are called.
...
**Strong delegate creates retain cycle**

A strong delegate property retains the delegate, which likely also references the owner, creating a memory leak.
...
1. **Always declare delegate properties as weak to prevent retain cycles.**
2. **Check respondsToSelector before calling optional delegate methods.**
3. **Set the delegate property before triggering any delegate callbacks.**
