---
title: "[Solution] Objective-C Compiler Error - How to Fix"
description: "Fix Objective-C compiler errors including syntax errors, type mismatches, missing imports, and ARC-related compilation failures."
...
- Missing import statements cause the compiler to not recognize class names, protocol names, or function declarations. Forward declarations with @class or @protocol may be needed.
- Type mismatches between Objective-C objects and C types cause compilation errors. For example, assigning a NSString to a char* requires explicit conversion.
- ARC-related errors occur when code violates ARC rules, such as calling retain/release directly or assigning incompatible pointer types without proper bridging.
- Missing method implementations for @required protocol methods cause compiler warnings (or errors with strict settings). The compiler requires all declared methods to be implemented.
...
2. **Incompatible pointer types assigning**
3. **ARC forbids explicit message send of release**
4. **Expected method definition**
...
```objective-c
#import "MyClass.h"
// Without import, MyClass is undeclared

// CORRECT: Import the header
#import "MyClass.h"
...
```objective-c
@interface MyVC : UIViewController <UITableViewDelegate>
@end

// Must implement @required methods
// or compiler warns/errors
```
...
**Missing import for used class**

A source file uses NSString but does not import Foundation. The compiler cannot find the class declaration and reports an undeclared identifier error.
...
A method expects a NSInteger parameter but receives an NSNumber. The implicit conversion fails at compile time.
...
**Protocol method not implemented**

The class declares conformance to a protocol but does not implement all @required methods. The compiler flags each missing method.
...
1. **Always check import statements when encountering undeclared identifier errors.**
2. **Use proper type conversions between Foundation types and C types with explicit cast methods.**
3. **Implement all @required protocol methods to satisfy compiler requirements.**
