...
tags: ["bad-any-cast", "std-any", "type-cast", "exception"]
weight: 50
---
...
- Casting a `std::any` to a different type than what was stored
- Using `std::any_cast` on an empty `std::any`
- Casting between incompatible types (e.g., `int` to `double`)
- Casting to a pointer type when a value is stored

## Example: Throwing std::bad_any_cast
...
## How to Fix: Use Type Checks and Matched Casts
...
- [std::bad_variant_access]({{< relref "/languages/cpp/bad-variant-access" >}}) — invalid `std::variant` access.
