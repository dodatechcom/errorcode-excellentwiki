---
title: "[Solution] Groovy AST Transformation Error - How to Fix"
description: "Fix Groovy AST transformation errors when compile-time code generation fails, produces incorrect AST, or conflicts with other transformations."
languages: ["groovy"]
...
- AST transformations modify the abstract syntax tree at compile time. Errors in the transformation code produce compilation errors that are difficult to diagnose because they occur in generated code.
- Multiple AST transformations may conflict if they modify the same AST nodes. The order of transformation application depends on annotation order and compiler settings.
- Type checking errors in AST-generated code may not point to the original source location, making debugging difficult. Source position must be explicitly set in generated AST nodes.
- Incorrect AST node construction causes runtime errors during compilation. The Groovy compiler expects specific AST node structures and crashes when they are malformed.
...
1. **Compilation error in AST-generated code**
2. **Conflicting AST transformations**
3. **Type checking error in generated code**
4. **Malformed AST node structure**
...
// CORRECT: Set source position
methodNode.setSourcePosition(originalASTNode)
```
...
```groovy
// Debug AST output
groovyc -Dgroovy.output.atd=true source.groovy
// Generates .atd file with AST structure

// Or use AST browser in IDE
```
...
```groovy
// CORRECT: Handle errors in transformation
@Override
void visit(ASTNode[] nodes, GeneratorContext context) {
    try {
        // Transform AST
    } catch (Exception e) {
        // Report meaningful error
        addError("Transformation failed: ${e.message}", nodes[0])
    }
}
```
...
**AST generates wrong code**

An AST transformation generates code that does not type-check. The compiler reports the error in the generated code, not the original source.
...
**Two transformations conflict**

Two AST transformations both modify the same method node. The second transformation overwrites changes from the first.
...
An AST transformation creates new AST nodes without setting their source position. Stack traces and error messages show line 0 or wrong locations.
...
1. **Use the AST browser in your IDE to inspect the generated AST and verify transformation output.**
2. **Always set source positions on generated AST nodes for meaningful error messages and stack traces.**
3. **Test AST transformations with compilation error cases to ensure graceful error reporting.**
...
- [COMPILER Error](/languages/groovy/groovy-compiler-error) — compilation
