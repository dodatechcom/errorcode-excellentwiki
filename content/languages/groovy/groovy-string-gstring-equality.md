---
title: "[Solution] Groovy String GString Equality Error - How to Fix"
description: "Fix Groovy String vs GString equality errors when comparing strings with different types that appear identical but are not equal."
languages: ["groovy"]
...
- In Groovy, GString and String with identical content are NOT equal. This is a common source of bugs when comparing interpolated strings with plain strings.
- GString.toString() returns a String, but the GString object itself is not equal to a String with the same content. Use .toString() before comparison.
- Map operations using mixed GString and String keys fail silently. The key is not found because GString key != String key even with identical content.
- Groovy truth treats both GString and String the same way (empty is falsy), but == comparison is type-sensitive.
...
1. **GString not equal to String with same content**
2. **Map lookup fails with GString key**
3. **Set contains duplicate-looking entries**
4. **String comparison returns unexpected false**
...
```groovy
def name = 'World'
def gstr = "Hello ${name}" // GString
def str = 'Hello World'    // String

println gstr == str  // false!
println gstr.toString() == str // true!
```
...
```groovy
def map = ['key': 1]
def gkey = "${'key'}" // GKey is GString
println map[gkey] // null!

println map[gkey.toString()] // 1
```
...
set << 'hello'
set << "${'hello'}" // GString
println set.size() // 2! Both in set

// CORRECT: Normalize types
set << 'hello'.toString()
set << "${'hello'}".toString()
```
...
```groovy
if (gstr.toString() == str) {
    println 'Equal!'
}

// Or use compareTo
if (gstr.toString().compareTo(str) == 0) {
    println 'Equal!'
}
```
...
**GString/String equality false**

A GString and a String with identical text content are compared with ==. The result is false because they are different types.
...
A GString is used to create a map entry. Later, a String with the same content is used to look up the entry. The lookup fails.
...
**Set appears to have duplicates**

A GString and a String with the same content are both added to a HashSet. The set contains both because they are not equal.
...
1. **Always compare GString and String using .toString() on both sides to ensure type-independent comparison.**
2. **Normalize map keys to String type before lookup when keys may be GString or String.**
3. **Use .toString() when adding strings to Sets or Maps that may contain mixed types.**
...
- [GSTRING Error](/languages/groovy/groovy-gstring-error) — GString issues
