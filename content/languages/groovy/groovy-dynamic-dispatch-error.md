---
title: "[Solution] Groovy Dynamic Dispatch Error - How to Fix"
description: "Fix Groovy dynamic method dispatch errors when method resolution fails at runtime due to missing methods, ambiguous overloads, or wrong argument types."
languages: ["groovy"]
...
- Groovy resolves methods at runtime based on the actual argument types, not the declared parameter types. This can select a different method than expected.
- MissingMethodException is thrown when no method matches the call signature. This includes the method name, argument count, and argument types.
- MethodMissingException is thrown when no method is found by the normal dispatch and the methodMissing method is not implemented on the class.
- Groovy dynamic dispatch may select Groovy-specific methods (like getAt, putAt) instead of standard Java methods when the naming conventions overlap.
...
1. **MissingMethodException - no matching method found**
2. **MethodMissingException - method not defined**
3. **Wrong method selected due to dynamic dispatch**
4. **Groovy meta-method shadowing Java method**
...
if (obj.respondsTo('nonExistentMethod')) {
    obj.nonExistentMethod()
}
```
...
new MyClass().unknown() // Prints message
```
...
def process(Object obj) { 'Object' }
def process(String s) { 'String' }

println process('hello') // 'String'
println process(42)       // 'Object'
```
...
list[0] // Calls list.getAt(0)
list << 'item' // Calls list.leftShift('item')

// These are Groovy-specific, not Java methods
```
...
A method is called that does not exist on the object. The Groovy runtime throws MissingMethodException instead of a compile-time error.
...
Two methods have similar signatures. Groovy dynamic dispatch selects a different method than the programmer intended based on runtime argument types.
...
**MethodMissing not implemented**

A class does not implement methodMissing and a non-existent method is called. The default behavior throws MissingMethodException.
...
1. **Use respondsTo() to check if a method exists before calling it dynamically.**
2. **Implement methodMissing for classes that support dynamic method invocation.**
3. **Be aware of Groovy operator overloading methods (getAt, putAt, leftShift) that may be selected instead of standard Java methods.**
