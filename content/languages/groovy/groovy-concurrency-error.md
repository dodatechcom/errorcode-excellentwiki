---
title: "[Solution] Groovy Concurrency Error - How to Fix"
description: "Fix Groovy concurrency errors including shared mutable state, closure thread safety, and GPars parallel execution issues."
languages: ["groovy"]
...
- Groovy closures are not inherently thread-safe. Shared mutable state accessed from multiple closures running in parallel causes race conditions.
- GPars DataFlow variables provide thread-safe communication but incorrect usage (reading before value is set) causes DataFlowVariables.PolicyException.
- Groovy's dynamic method dispatch is not thread-safe for metaclass modifications. Adding methods to a metaclass from multiple threads causes concurrent modification issues.
- List and Map operations in Groovy are not atomic. Concurrent modifications to shared collections cause ConcurrentModificationException or data corruption.
...
1. **Race condition on shared mutable state**
2. **DataFlowVariables.PolicyException**
3. **ConcurrentModificationException**
4. **Concurrent metaclass modification**
...
// Reading before setting causes exception!

// CORRECT: Set before reading
def result = new DataFlowVariable()
result << 42 // Set value
...
```groovy
// Metaclass modification - not thread-safe!
// Do this at startup, not in parallel code
...
**Race condition in parallel collection**

Multiple threads increment a shared counter. Some increments are lost because the read-modify-write is not atomic.
...
**DataFlow read before set**

A DataFlowVariable is read before a value is set. The policy exception prevents deadlock but indicates a logic error.
...
One thread iterates a list while another thread adds elements. ConcurrentModificationException is thrown.
...
1. **Use atomic variables (AtomicInteger, AtomicBoolean) for simple shared counters and flags.**
2. **Use ConcurrentHashMap or synchronized blocks for thread-safe collection access.**
3. **Complete all metaclass modifications at startup before any concurrent code begins.**
