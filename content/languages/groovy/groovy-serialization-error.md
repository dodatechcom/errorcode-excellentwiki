---
title: "[Solution] Groovy Serialization Error - How to Fix"
description: "Fix Groovy serialization errors when objects cannot be serialized, deserialized, or when class version mismatches break serialized data."
languages: ["groovy"]
...
- Classes must implement Serializable to be serialized. Attempting to serialize non-Serializable objects throws NotSerializableException.
- Transient fields are excluded from serialization. If a transient field is needed after deserialization, it must be reinitialized in readObject or readResolve.
- Class version changes (adding/removing fields) break deserialization of previously serialized objects. serialVersionUID must be explicitly set for version compatibility.
- Groovy objects may contain closure references that are not serializable. Serializing objects with closure properties fails unless closures are marked transient.
...
1. **NotSerializableException - class not serializable**
2. **InvalidClassException - serialVersionUID mismatch**
3. **StreamCorruptedException - corrupted serialized data**
4. **Closure not serializable in object**
...
new ObjectOutputStream(new FileOutputStream('file'))
    .writeObject(new MyClass(name: 'test', age: 25))
```
...
class MyClass implements Serializable {
    String name
    transient Thread worker // Not serialized

    private void readObject(ObjectInputStream ois) {
        ois.defaultReadObject()
        worker = Thread.currentThread() // Reinitialize
    }
}
```
...
class MyClass implements Serializable {
    String name
    transient Closure processor // Not serialized
}
```
...
A class is passed to ObjectOutputStream.writeObject but does not implement Serializable. The serialization attempt throws NotSerializableException.
...
A serialized object is saved with serialVersionUID=1 but the class definition now has serialVersionUID=2. Deserialization throws InvalidClassException.
...
**Closure in serialized object**

A Groovy class with a closure property is serialized. The closure is not serializable and the entire serialization fails.
...
1. **Always implement Serializable and declare serialVersionUID for classes that will be serialized.**
2. **Mark non-serializable fields (closures, threads, database connections) as transient.**
3. **Implement readObject to reinitialize transient fields after deserialization.**
