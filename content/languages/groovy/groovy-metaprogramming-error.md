---
title: "[Solution] Groovy Metaprogramming Error - How to Fix"
description: "Fix Groovy metaprogramming errors when using dynamic method dispatch, ExpandoMetaClass, and runtime method generation."
languages: ["groovy"]
...
- Groovy metaprogramming allows adding methods and properties to classes at runtime. Incorrect metaclass modifications affect all instances of a class, potentially breaking unrelated code.
- ExpandoMetaClass modifications are global. Adding a method to String.metaClass affects every String in the JVM, including third-party libraries.
- Dynamic method dispatch via methodMissing and propertyMissing can cause infinite recursion if the fallback does not handle the missing method properly.
- Thread safety issues arise when metaclass modifications happen concurrently. The metaclass is shared across threads, and concurrent modifications may cause race conditions.
...
1. **MissingMethodException - method not found on metaclass**
2. **StackOverflowError from methodMissing recursion**
3. **Concurrent modification of metaclass**
4. **Global metaclass change affects all instances**
...
class MyClass {
    def methodMissing(String name, args) {
        this.invokeMethod(name, args) // Recursion!
    }
}

// CORRECT: Handle or delegate to super
class MyClass {
    def methodMissing(String name, args) {
...
```groovy
// WRONG: Modifying global metaclass
String.metaClass.prettyPrint = { ->
...
def obj = new MyClass()
obj.metaClass.prettyPrint = { -> 'custom' }
```
...
```groovy
// ExpandoMetaClass for class
class MyClass {}
MyClass.metaClass.newMethod = { -> 'hello' }

// CORRECT: Check before adding
if (!MyClass.metaClass.respondsTo('newMethod')) {
    MyClass.metaClass.newMethod = { -> 'hello' }
}
```
...
```groovy
// Thread-safe metaclass modification
synchronized(MyClass.metaClass) {
    MyClass.metaClass.newMethod = { -> 'hello' }
}
```
...
**methodMissing infinite recursion**

methodMissing calls this.invokeMethod which routes back to methodMissing, creating infinite recursion and StackOverflowError.
...
**Global String metaclass change**

A library modifies String.metaClass to add a helper method. This affects every String in the application, including internal JDK strings.
...
**Concurrent metaclass modification**

Two threads simultaneously add methods to a class metaclass. One method addition is lost or the metaclass state becomes corrupted.
...
1. **Always check for infinite recursion in methodMissing and propertyMissing by calling the appropriate fallback or throwing MissingMethodException.**
2. **Prefer per-instance metaclass modifications over class-level metaclass changes.**
3. **Synchronize metaclass modifications or use runtime.exec() for thread-safe dynamic additions.**
...
- [MEMORY Error](/languages/groovy/groovy-memory-error) — memory issues
