---
title: "[Solution] Groovy Builder DSL Error - How to Fix"
description: "Fix Groovy Builder DSL errors when using MarkupBuilder, NodeBuilder, or custom builders with incorrect nesting, missing closures, or wrong element structure."
languages: ["groovy"]
...
- MarkupBuilder requires proper XML nesting. Elements not properly closed or nested incorrectly produce malformed XML that downstream parsers reject.
- Builder methods use closures for child elements. Forgetting the closure means the element has no children, which may not match the expected XML structure.
- Attribute names in builders must be valid XML identifiers. Using reserved words, spaces, or special characters in attribute names causes XML generation errors.
- Builder indentation and formatting depend on the PrintWriter settings. Incorrect settings may produce output that does not match expected formatting.
...
1. **Malformed XML output from MarkupBuilder**
2. **Missing child elements in builder output**
3. **Invalid attribute name in builder**
4. **Builder output not matching expected format**
...
```groovy
// WRONG: Missing closure for children
def xml = new MarkupBuilder()
xml.root
xml.child // No children!

// CORRECT: Use closure for nesting
xml.root {
    child('value')
}
```
...
def xml = new MarkupBuilder()
xml.root('class': 'my-class') // 'class' is reserved

// CORRECT: Use mkp.yield or escape
xml.root('my-class': 'value')
```
...
// Indented output
def xml = new MarkupBuilder()
xml.indentPrinter = new IndentPrinter(
...
MarkupBuilder produces XML that is not well-formed because elements are not properly nested or closed. Downstream XML parsers reject the output.
...
**Empty elements in output**

Builder methods called without closures produce self-closing tags instead of elements with children. The expected structure has child elements.
...
A builder attribute uses a Groovy keyword or XML reserved word as its name, causing the builder to throw an exception or produce invalid output.
...
1. **Always use closures for builder methods that should contain child elements.**
2. **Test builder output with an XML parser to verify well-formedness.**
3. **Use mkp.yieldUnescaped for raw content and escape attribute names that conflict with reserved words.**
