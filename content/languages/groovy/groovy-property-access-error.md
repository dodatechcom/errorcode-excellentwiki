---
title: "[Solution] Groovy Property Access Error - How to Fix"
description: "Fix Groovy property access errors when accessing non-existent properties, using incorrect getter/setter conventions, or encountering Groovy JavaBean property resolution issues."
languages: ["groovy"]
...
- Groovy resolves property access (obj.prop) to a getter method (getProp()) automatically. If neither the property nor the getter exists, a MissingPropertyException is thrown.
- Groovy allows property access on Maps using dot notation. This can cause confusion when Map keys shadow actual object properties.
- Boolean properties in Groovy use 'is' prefix (isActive()) instead of 'get' prefix (getActive()). Confusing these conventions causes property resolution failures.
- Groovy categories can add properties temporarily within a specific scope. Properties added via categories are not visible outside the category block.
...
1. **MissingPropertyException - property not found**
2. **Map key shadowing object property**
3. **Boolean property getter convention mismatch**
4. **Category property not visible outside scope**
...
```groovy
// WRONG: No property or getter
def obj = new MyClass()
println obj.nonExistent // MissingPropertyException

// CORRECT: Define property or getter
class MyClass {
    String name
}
```
...
// Map dot notation
def map = [name: 'test']
println map.name // 'test'

// But if map has actual objects:
def map = [obj: new MyClass(name: 'test')]
println map.obj.name // 'test'
```
...
// Category scope
use(MyCategory) {
    println 'hello'.customProp // Works
}
// println 'hello'.customProp // Fails outside
```
...
**MissingPropertyException on object**

Code accesses a property that does not exist on the object and no getter method is defined. Groovy throws MissingPropertyException.
...
**Map dot notation confusion**

A Map is used with dot notation for property access. The Map keys behave as pseudo-properties, which can shadow actual object properties.
...
**Boolean property convention**

A boolean property is defined as 'active' but code calls getActive() instead of isActive(). The getter is not found.
...
1. **Define all accessible properties explicitly in the class or use @Canonical for automatic generation.**
2. **Use bracket notation (map['key']) instead of dot notation for Map access to avoid confusion with object properties.**
3. **Follow Groovy boolean property conventions: use isActive() for boolean active property.**
...
- [METAPROGRAMMING Error](/languages/groovy/groovy-metaprogramming-error) — metaprogramming
