---
title: "[Solution] Groovy Closure Variable Capture Error - How to Fix"
description: "Fix Groovy closure variable capture errors including unexpected variable values, owner delegation, and closure scope issues."
languages: ["groovy"]
...
# Closure Variable Capture Error

This error is one of the most frequently encountered issues when developing with groovy. It affects programs of all sizes, from small utility scripts to large-scale production systems. Recognizing this error early and understanding its root cause can save hours of debugging and prevent data corruption or security vulnerabilities.
...
- Groovy closures capture variables by reference. Changes to the variable in the closure are visible outside, and vice versa. This can cause unexpected behavior if the variable is modified after closure creation.
- The delegate of a closure determines property resolution order. If the delegate is set to an object, property lookups may resolve on the delegate instead of the enclosing scope.
- Closures in loops capture the loop variable by reference. All closures share the same variable, which has the final loop value when the closures execute.
- The owner of a closure is the enclosing class or closure. delegate vs owner resolution can cause confusion about which object a method call resolves to.
...
1. **Closure captured wrong variable value**
2. **Delegate property shadows local variable**
3. **All closures in loop share same value**
4. **Owner vs delegate confusion**
...
list.each { i ->
    ...
    closures.each { it() } // All print 4!
}

// CORRECT: Capture by value
closures << { def captured = j; { println captured } }()
```
...
```groovy
// Delegate affects property lookup
def x = 10
closure = { x }
closure.delegate = [x: 20]
println closure() // Prints 20!

// CORRECT: Set delegate resolution strategy
closure.delegate = [x: 20]
...
list.each { it() } // All print 5!

// CORRECT: Capture in new scope
for (i in 1..5) {
    def captured = i
    list << { println captured }
}
```
...
```groovy
// OWNER_FIRST vs DELEGATE_FIRST
closure.resolveStrategy = Closure.OWNER_FIRST
// Check owner first, then delegate

closure.resolveStrategy = Closure.DELEGATE_FIRST
// Check delegate first, then owner
```
...
Multiple closures created in a loop all reference the same variable. By the time they execute, the variable has the final loop value.
...
**Delegate property shadowing**

A closure's delegate has a property with the same name as a local variable. The delegate property shadows the local variable due to resolution strategy.
...
**Closure modifies outer variable unexpectedly**

A closure modifies a captured variable, changing the value for all other closures that reference the same variable.
...
1. **Capture loop variables in a new local variable inside the loop body to give each closure its own copy.**
2. **Set Closure.OWNER_FIRST as the resolve strategy to prefer the enclosing scope over the delegate.**
3. **Be aware that closures capture variables by reference, not by value. Mutable captured state is shared.**
...
- [RUNTIME Error](/languages/groovy/groovy-runtime-error) — runtime issues
- [THREAD Error](/languages/groovy/groovy-thread-error) — threading
- [TYPE Error](/languages/groovy/groovy-type-error) — type issues
