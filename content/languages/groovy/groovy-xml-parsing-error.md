---
title: "[Solution] Groovy XML Parsing Error - How to Fix"
description: "Fix Groovy XML parsing errors when using XmlSlurper, XmlParser, or MarkupBuilder with malformed XML, namespace issues, or encoding problems."
languages: ["groovy"]
...
- XmlSlurper requires well-formed XML. Malformed XML (unclosed tags, mismatched tags, invalid characters) throws SAXParseException.
- XML namespaces require proper handling with withNamespace or namespace-aware parsing. Ignoring namespaces causes element lookup failures.
- Character encoding mismatches between the XML declaration and the actual content cause garbled text or parsing errors.
- Large XML documents parsed entirely into memory may cause OutOfMemoryError. Streaming parsers (XmlSlurper with SAX) are more memory-efficient.
...
1. **SAXParseException - malformed XML**
2. **Namespace not found in XML path**
3. **Character encoding mismatch**
4. **OutOfMemoryError from large XML**
...
```groovy
// WRONG: Malformed XML
def xml = '<root><child>value</root>' // Missing </child>
// SAXParseException
...
```groovy
// XML with namespaces
def xml = '<root xmlns:ns="http://example.com"><ns:child/></root>'
def parsed = new XmlSlurper().parseText(xml)
println parsed.depthFirst().find { it.name().localPart == 'child' }
```
...
An XML document has an unclosed tag. XmlSlurper.parseText throws SAXParseException with a message indicating the location of the error.
...
An XML element is in a namespace but the GPath expression does not account for the namespace. The element is not found.
...
The XML declaration specifies UTF-8 but the actual content contains characters from a different encoding. Characters are garbled or cause parsing errors.
...
1. **Validate XML with a schema or DTD before parsing to catch structural errors early.**
2. **Use namespace-aware parsing when working with XML documents that use namespaces.**
3. **Use streaming parsers (SAX) for large XML documents to avoid loading the entire document into memory.**
...
- [JSON Error](/languages/groovy/groovy-json-parsing-error) — JSON issues
