---
title: "[Solution] Groovy Default Parameter Error - How to Fix"
description: "Fix Groovy default parameter errors when optional parameters cause method resolution ambiguity or incorrect default values."
languages: ["groovy"]
...
- Groovy methods with default parameters can be called with or without the optional arguments. The compiler selects the method based on argument count, which may be ambiguous.
- Default parameter values are evaluated at compile time for primitives but at runtime for objects. Null default values for object parameters may cause NullPointerException when accessed.
- Multiple methods with default parameters may have overlapping signatures. The compiler may report an ambiguous method call error when it cannot determine which method to call.
- Java callers cannot use Groovy default parameters. Default parameters are a Groovy-specific feature that does not generate Java-compatible overloaded methods.
...
1. **Ambiguous method call - multiple matching methods**
2. **NullPointerException from default parameter**
3. **Java caller cannot use Groovy defaults**
4. **Wrong method selected due to defaults**
...
```groovy
// WRONG: Ambiguous
def process(String a, String b = 'default') { }
def process(String a) { }
// Which one to call?

// CORRECT: Single method with defaults
def process(String a, String b = 'default') {
    println "${a} ${b}"
}
```
...
```groovy
// Default null value
def greet(String name = null) {
    println "Hello ${name.toUpperCase()}" // NPE!
}

// CORRECT: Check for null
def greet(String name = null) {
    println "Hello ${name?.toUpperCase() ?: 'World'}"
}
```
...
```groovy
// Java cannot use Groovy defaults
// Groovy:
def process(String a, int b = 0) { }

// Java must call with all args:
// process("hello", 0) - cannot omit b
```
...
def configure(Map params = [:]) {
    def name = params.name ?: 'default'
    def size = params.size ?: 10
}

configure(name: 'test') // Uses defaults
```
...
Two methods differ only in the number of default parameters. The compiler cannot determine which method to call when called with intermediate argument count.
...
**Null default causes NPE**

A default parameter value is null. Code that accesses properties on the parameter without null checking throws NullPointerException.
...
**Java-Groovy interop failure**

A Java class tries to call a Groovy method with default parameters. The Java compiler does not see the default parameter overloads and reports a compilation error.
...
1. **Avoid defining multiple methods with overlapping default parameter signatures.**
2. **Always handle null default values with safe navigation or null checks.**
3. **Use Map-based parameters for APIs that need to be callable from both Java and Groovy.**
