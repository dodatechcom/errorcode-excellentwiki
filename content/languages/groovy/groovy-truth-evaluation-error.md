---
title: "[Solution] Groovy Truth Evaluation Error - How to Fix"
description: "Fix Groovy truth evaluation errors when boolean conditions evaluate unexpectedly due to Groovy's special truth rules for non-boolean types."
languages: ["groovy"]
...
- Groovy truth differs from Java truth. Empty strings, empty collections, zero, and null are all falsy in Groovy but may be truthy in other languages.
- Custom classes can define a asBoolean() method or boolean getXxx() method to control their truth value. Unexpected truth results may indicate a missing or incorrect truth definition.
- Groovy truth evaluation calls asBoolean() implicitly. If the method is not defined, the default truth rules apply. Adding a custom asBoolean() can change the behavior unexpectedly.
- Boolean wrapper types (Boolean) follow the same truth rules as primitives (boolean). Null Boolean is falsy, Boolean.FALSE is falsy, Boolean.TRUE is truthy.
...
1. **Unexpected falsy evaluation of non-null value**
2. **Empty collection evaluated as false**
3. **Custom class truth value incorrect**
4. **Groovy truth differs from expected Java behavior**
...
assert !' '         // Blank string = true!
assert !0           // Zero = false
assert ![]          // Empty list = false
assert ![:]         // Empty map = false
assert null == false // null = false
```
...
def v = new MyValue(value: 0)
assert !v  // Falsy because value <= 0
```
...
if (str?.trim()) {
    // str is non-null and non-blank
}

// Not the same as:
if (str) {
    // str is non-null but could be blank
```
...
// For other types, use explicit comparison
if (myList != null && !myList.isEmpty()) {
    // Explicitly checks emptiness
}
```
...
**Blank string is truthy**

A string containing only whitespace is evaluated in a boolean context. Unlike an empty string, a blank string is truthy in Groovy.
...
**Custom asBoolean changes behavior**

A class defines asBoolean() that returns false for certain valid states. Code that checks if object is truthy silently skips processing for those states.
...
An empty ArrayList is evaluated in a boolean condition. Groovy treats it as false, which may not be the expected behavior.
...
1. **Learn the Groovy truth rules: null, empty strings, 0, empty collections are all falsy.**
2. **Use explicit comparisons (== null, .isEmpty()) when Groovy truth might not match your intent.**
3. **Define asBoolean() on custom classes to provide meaningful truth evaluation behavior.**
