---
title: "[Solution] Groovy NullPointerException Missing Null Check - How to Fix"
description: "Fix Groovy NullPointerException errors caused by missing null checks before accessing object methods or properties on null references."
languages: ["groovy"]
...
- Groovy allows method calls on null references in many contexts through its safe navigation operator, but not all code paths use it. Accessing a property on null throws NullPointerException.
- The safe navigation operator (?.) returns null instead of throwing NPE when the receiver is null, but it must be explicitly used in the code.
- Groovy truth evaluation treats null as falsy, but explicitly calling methods on null still throws NPE. The truth operator only works in boolean contexts.
- Map and list access with null keys or null values may produce unexpected null results that propagate through chained method calls.
...
1. **NullPointerException at line X**
2. **Cannot invoke method on null object**
3. **Cannot get property of null value**
4. **Safe navigation operator not used**
...
// WRONG: Direct access
List items = getItems()
items.add('item') // NPE if getItems() returns null

// CORRECT: Check or default
List items = getItems() ?: []
items.add('item')
```
...
```groovy
// WRONG: Chained calls
result = service?.getData()?.process()
// But service itself could be null

// CORRECT: Full null-safe chain
result = service?.getData()?.process() ?: defaultValue
```
...
```groovy
// Use Groovy truth in conditions
if (user?.name) {
    // user and name are both non-null
...
A chained method call like user.getAddress().getCity() throws NPE because getAddress() returns null. The safe navigation operator must be used at each step.
...
A map lookup returns null for a missing key. The null value is then passed to a method that does not accept null, causing NPE.
...
**Null in collection iteration**

A collection contains null elements. Iterating and calling methods on elements without null checks throws NPE.
...
1. **Use the safe navigation operator (?.) for all method calls that may have null receivers.**
2. **Use the Elvis operator (?:) to provide default values for potentially null expressions.**
3. **Initialize collections and objects before use to prevent null reference issues.**
...
- [TYPE Error](/languages/groovy/groovy-type-error) — type mismatches
- [NULL SAFETY Error](/languages/groovy/groovy-null-safety) — null handling
