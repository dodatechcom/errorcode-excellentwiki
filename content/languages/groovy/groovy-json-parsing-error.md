---
title: "[Solution] Groovy JSON Parsing Error - How to Fix"
description: "Fix Groovy JSON parsing errors when using JsonSlurper, JsonOutput, or JSON-to-object conversion with malformed data or type mismatches."
languages: ["groovy"]
...
- JsonSlurper.parseText() expects valid JSON syntax. Malformed JSON (missing quotes, trailing commas, single quotes) throws JsonException.
- JSON numbers may lose precision when parsed to BigDecimal or Integer. Very large numbers overflow int/long and require explicit type handling.
- JSON null values map to Groovy null. Accessing properties on a null JSON element throws NullPointerException.
- JsonSlurper returns LinkedHashMap for objects and ArrayList for arrays. These are not the target domain classes, requiring explicit type conversion.
...
1. **JsonException - malformed JSON**
2. **NumberFormatException from JSON number**
3. **NullPointerException from JSON null**
4. **ClassCastException: LinkedHashMap to domain class**
...
def json = JsonSlurper.parseText("{'name': 'test'}")
...
```groovy
// Large numbers
def json = '{"id": 9999999999999999}'
def parsed = JsonSlurper.parseText(json)
println parsed.id.class // BigDecimal

// CORRECT: Handle precision
def id = parsed.id.toLong()
```
...
def json = '{"name": null}'
def parsed = JsonSlurper.parseText(json)
println parsed.name.toUpperCase() // NPE!

// CORRECT: Null-safe access
println parsed.name?.toUpperCase()
```
...
Groovy allows single-quoted strings, but JSON requires double quotes. Using Groovy-style strings in JSON input causes a parse error.
...
A JSON number exceeds the precision of int or long. The parsed value loses precision or throws an exception.
...
**JSON null causes NPE**

A JSON field contains null. The code accesses a method on the null value without null checking, causing NullPointerException.
...
1. **Always use valid JSON syntax with double quotes for property names and string values.**
2. **Use JsonSlurper.parseType() or manual conversion for domain-specific types.**
3. **Handle null values from JSON with safe navigation operator before accessing properties.**
