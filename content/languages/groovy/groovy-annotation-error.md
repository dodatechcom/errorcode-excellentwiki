---
title: "[Solution] Groovy Annotation Processing Error - How to Fix"
description: "Fix Groovy annotation processing errors when custom annotations fail to generate code, produce incorrect output, or conflict with Groovy-specific annotations."
languages: ["groovy"]
...
- Groovy annotation processors run during compilation. If the processor code has errors, the compilation fails with obscure error messages from the annotation processing round.
- Groovy-specific annotations (@Canonical, @TupleConstructor, @Builder) generate code at compile time. Errors in the generated code may not clearly indicate the annotation as the source.
- Annotation processors that depend on Java-specific APIs may fail when processing Groovy source files because the AST structure differs from Java.
- Multiple annotation processors may conflict if they modify the same AST nodes. The order of processor execution depends on the META-INF/services configuration.
...
1. **Annotation processor compilation error**
2. **Groovy annotation generates wrong code**
3. **Processor fails on Groovy source files**
4. **Conflicting annotation processors**
...
// CORRECT: Handle Groovy AST differences
```
...
```groovy
@Canonical // Generates toString, equals, hashCode
class Point { int x; int y }

// If generated code has errors:
// Check that all fields have accessible getters
```
...
```groovy
// Register processor in META-INF/services
// javax.annotation.processing.Processor
com.example.MyProcessor

// CORRECT: Ensure proper service file
```
...
// If processors conflict:
// Run them in separate compilation rounds
```
...
**Annotation processor fails on Groovy**

The annotation processor expects Java AST structure but receives Groovy AST, which has different node types and relationships.
...
An annotation generates code that does not compile due to missing imports, wrong types, or Groovy-specific syntax that the generated code does not use.
...
**Processor order conflict**

Two annotation processors modify the same class. The order they execute determines which changes are applied, causing non-deterministic results.
...
1. **Test annotation processors with both Java and Groovy source files.**
2. **Use Groovy-aware AST transformations instead of Java annotation processors when targeting Groovy code.**
3. **Debug annotation processor output by examining the generated source files in the build output directory.**
...
- [AST Error](/languages/groovy/groovy-ast-error) — AST issues
