---
title: "[Solution] Groovy GString Type Error - How to Fix"
description: "Fix Groovy GString type mismatch errors when GString is used where String is expected, causing type conversion failures or method resolution issues."
languages: ["groovy"]
...
- GString and String are different types in Groovy. Methods that expect String parameters may fail when passed a GString because Groovy performs strict type checking in some contexts.
- GString interpolation creates a GStringImpl object. When passed to Java methods expecting String, automatic toString() conversion occurs, but this may fail in generic type contexts.
- Map keys and set elements that are GString will not match String keys/values. ["key": 1] and [('key'): 1] are different map entries.
- GString.toString() may produce different results than expected if the interpolated expression has a custom toString() implementation.
...
1. **GString cannot be cast to String**
2. **No such method: find(String) for GString**
3. **Map key mismatch: GString vs String**
4. **Type mismatch in generic parameter**
...
// May fail:
someJavaMethod(greeting) // Expects String

// CORRECT: Explicit conversion
someJavaMethod(greeting.toString())
```
...
```groovy
// Map key mismatch
def map1 = ["key": 1] // String key
def map2 = [(${variable}): 1] // GString key
// map1 and map2 have different keys!

// CORRECT: Use String keys
map2 = [variable.toString(): 1]
```
...
def s1 = 'Hello' // String
def s2 = "Hello" // GString (no interpolation)

def map = [:]
map[s1] = 1
map[s2] = 2 // Different key!
```
...
list.each { String s -> // Explicit type
    println s
}
```
...
**GString passed to Java method**

A GString is passed to a Java method that expects String. In most cases automatic conversion works, but in generic type contexts it fails.
...
A GString is used as a map key. A subsequent lookup using a String key fails because GString and String are different keys.
...
**GString in Set comparison**

A GString and a String with the same content are both added to a Set. They are not considered equal, so the Set contains both elements.
...
1. **Use single quotes for strings that do not require interpolation to ensure they are String type.**
2. **Convert GString to String explicitly with .toString() when passing to Java methods or using as map keys.**
3. **Be aware that GString and String with identical content are NOT equal in Groovy.**
