---
title: "[Solution] Groovy Type Coercion Error - How to Fix"
description: "Fix Groovy type coercion errors when automatic type conversion produces unexpected results or fails at runtime."
languages: ["groovy"]
...
- Groovy performs automatic type coercion in many contexts. This can produce unexpected results when a Number is coerced to a String or vice versa.
- Groovy truth coercion treats non-null, non-empty values as true. This differs from Java where only boolean values are truthy.
- Dynamic typing in Groovy defers type checking to runtime. A variable declared as def can hold any type, but accessing type-specific methods may fail at runtime.
- Method overloading resolution in Groovy uses runtime types. Coercion may select a different overloaded method than expected based on implicit type conversions.
...
1. **GroovyCastException - cannot cast type**
2. **Unexpected type coercion result**
3. **Method resolution ambiguity from coercion**
4. **Runtime type mismatch with def**
...
```groovy
// Type coercion
def x = '42' + 1  // '421' (String concat)
def y = '42' as int // 42 (explicit coercion)

// WRONG: Unexpected coercion
def result = 5 / 2 // 2.5 (BigDecimal)
def intResult = 5.intdiv(2) // 2 (integer)
```
...
```groovy
// Groovy truth coercion
assert 'hello'  // true (non-empty string)
assert 0         // false (zero)
assert [1, 2]    // true (non-empty list)
...
```groovy
// Explicit type for def
def value = 'hello'
println value.size() // OK: String method
println value + 5    // 'hello5'

// But:
println value.toInteger() // NumberFormatException
```
...
```groovy
// Use @TypeChecked for compile-time safety
...
**Unexpected division result**

Integer division in Groovy returns BigDecimal by default. Code expecting integer result gets a decimal value, causing downstream errors.
...
A numeric variable is concatenated with a string using +. The number is coerced to a string instead of performing arithmetic.
...
Two overloaded methods accept different types. Groovy's implicit coercion allows both to match, causing an ambiguous method call error.
...
1. **Use intdiv() for integer division instead of / which returns BigDecimal.**
2. **Use @TypeChecked or @CompileStatic for compile-time type checking in critical code paths.**
3. **Use explicit type conversion (as keyword) instead of relying on implicit coercion.**
