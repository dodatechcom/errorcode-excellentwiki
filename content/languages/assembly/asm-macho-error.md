---
title: "[Solution] Assembly Mach-O Binary Format Error — How to Fix"
description: "Fix assembly Mach-O format errors when building macOS executables with incorrect load commands, segment layout, or symbol tables."
languages: ["assembly"]
...
- Mach-O is the executable format for macOS/iOS. It uses different load commands, segment layout, and symbol conventions than ELF.
- macOS requires underscore-prefixed global symbols (_main instead of main) to match C symbol naming.
- macOS syscall numbers differ: 0x2000000 + Linux number. Using Linux numbers performs wrong syscalls.
- Invalid Mach-O magic (0xFEEDFACF for 64-bit) or CPU type causes kernel rejection.
...
1. **Mach-O Header Error — Invalid Magic Number or cputype**
2. **LC_SEGMENT Error — Invalid Segment Command or Section Alignment**
3. **Symbol Table Error — nlist Entry Mismatch with String Table**
4. **codesign Error — Invalid Mach-O Signature or Entitlements**
...
```assembly
; WRONG: ELF on macOS
; nasm -f elf64 program.asm  -> Wrong!

    ; CORRECT: Mach-O format
...
```assembly
; WRONG: Linux syscalls on macOS
    mov rax, 60        ; Linux exit
    syscall

    ; CORRECT: macOS syscalls
    mov rax, 0x2000001  ; macOS exit
    xor rdi, rdi
    syscall
```
...
```assembly
; CORRECT: Complete macOS program
section .data
    msg: db 'Hello!', 10
    msg_len equ $ - msg
section .text
...
**Porting Linux asm to macOS**

Copying Linux x86_64 code and using -f elf64 on macOS produces an ELF binary the macOS linker rejects.

**Missing underscore on C symbols**

Assembly defines main without underscore. C compiler on macOS looks for _main — undefined symbol.

**Wrong syscall numbers after OS migration**

Code written for Linux uses syscall 4 (write) but macOS syscall 4 is a different syscall entirely.
...
1. **Use -f macho64 for 64-bit macOS, -f macho32 for 32-bit.**
2. **Prefix all global symbols with underscore (_).**
3. **Use macOS syscall numbers: 0x2000000 + Linux number.**
...
- [System Call Error](/languages/assembly/asm-syscall-error) — wrong syscall numbers
