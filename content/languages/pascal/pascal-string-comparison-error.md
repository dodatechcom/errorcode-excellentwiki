---
title: "[Solution] Pascal String Comparison Error - How to Fix"
description: "Fix Pascal string comparison errors when comparing strings with wrong operators, case sensitivity issues, or locale-dependent behavior."
languages: ["pascal"]
...
- The = operator compares strings byte-by-byte. In Delphi 2009+, string comparison is Unicode-aware. Comparing AnsiString with UnicodeString may produce unexpected results.
- Case-sensitive comparison is the default. Use SameText() for case-insensitive comparison. Comparing with UpCase() only works for ASCII characters.
- Empty strings ('') and nil strings behave differently in some contexts. An empty string has Length 0 but is not nil. nil string access may raise exceptions.
- String comparison with = returns True only if both strings are identical. Two strings that look the same but have different encodings may not compare as equal.
...
1. **Case-sensitive comparison when case-insensitive needed**
2. **AnsiString vs UnicodeString comparison issues**
3. **Nil string vs empty string confusion**
4. **Different encodings compare as not equal**
...
begin
  A := 'Hello';
  B := 'hello';
  if A = B then // False - case sensitive
    ShowMessage('Equal');
end;

// CORRECT: Use SameText for case-insensitive
...
begin
  A := '';
  B := nil;
  // A = B may not be True in all contexts
  if Length(B) = 0 then
    ShowMessage('B is empty');
end;
```
...
// Or use SameText for Boolean result
```
...
**Case sensitivity causes false inequality**

Strings that differ only in case compare as not equal with the = operator. Use SameText() for case-insensitive comparison.
...
String comparison order depends on the system locale. Characters that compare equally in one locale may differ in another.
...
**Unicode vs Ansi string comparison**

Comparing AnsiString with UnicodeString may produce unexpected results due to encoding conversion differences.
...
1. **Use SameText() for case-insensitive string comparison.**
2. **Be aware of string encoding when comparing AnsiString and UnicodeString.**
3. **Use CompareText() when you need ordering comparison (<, >) with case-insensitivity.**
...
- [LOCALE Error](/languages/pascal/pascal-locale-error) — locale issues
