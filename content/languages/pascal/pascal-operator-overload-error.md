---
title: "[Solution] Pascal Operator Overloading Error - How to Fix"
description: "Fix Pascal operator overloading errors when defining custom operators for record types, including missing operators and type mismatches."
languages: ["pascal"]
...
- Operator overloading is only available for record types in Object Pascal. Classes cannot overload operators. Attempting to overload operators on a class raises a compiler error.
- Each overloaded operator must have the correct parameter and return types. For binary operators, both parameters must be the same record type.
- The implicit operator allows automatic type conversion. If multiple implicit operators could apply, the compiler raises an ambiguity error.
- Not all operators can be overloaded. Only the standard arithmetic, comparison, and bitwise operators are supported.
...
1. **Operator overloading on class type - compiler error**
2. **Operator parameter type mismatch**
3. **Ambiguous implicit operator conversion**
4. **Missing required operator overload**
...
type
  ...
  class operator Add(A: TPoint; B: Integer): TPoint;
  // Error: parameter types must match

// CORRECT: Both parameters same type
...
type
  ...
  class operator Implicit(A: Integer): TIntWrapper;
  class operator Implicit(A: TIntWrapper): Integer;
  // Both directions - may cause ambiguity
```
...
type
  ...
  // Both operators needed for complete comparison
```
...
Object Pascal only supports operator overloading on record types. Classes cannot have overloaded operators.
...
Binary operator parameters must both be the same record type. Mixing types raises a compiler error.
...
**Implicit operator ambiguity**

Multiple implicit operators could apply for a conversion, causing the compiler to reject the ambiguous conversion.
...
1. **Only overload operators on record types, not classes.**
2. **Ensure both parameters of a binary operator are the same record type.**
3. **Be careful with implicit operators to avoid ambiguity. Prefer explicit conversion methods.**
