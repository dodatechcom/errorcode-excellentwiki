---
title: "[Solution] Pascal Array Bounds Error - How to Fix"
description: "Fix Pascal array index out of bounds errors when accessing arrays with invalid indices."
languages: ["pascal"]
...
- Arrays declared with a fixed range (array[1..100] of Integer) require indices within that range. Accessing index 0 or 101 raises an ERangeCheckError.
- Dynamic arrays (array of Integer) are indexed from 0 to Length-1. Accessing beyond this range causes an access violation.
- Open array parameters in procedures accept any size array but the index range is 0 to High(OpenArray). Accessing beyond this range is undefined.
- With {$R+} range checking enabled, invalid array indices are caught at runtime. Without it, the access is silent and may corrupt adjacent memory.
...
1. **ERangeCheckError - range check failure**
2. **Access violation - array index out of bounds**
3. **Index 0 on array declared from 1**
4. **Dynamic array accessed after being freed**
...
begin
  A[0] := 5;  // Range check error
  A[11] := 5; // Range check error
end;

// CORRECT: Stay within declared bounds
  A[1] := 5;
  A[10] := 5;
```
...
begin
  SetLength(A, 5);
  A[5] := 42;
  // Access violation - index 5 is out of bounds
end;

// CORRECT: Use indices 0..4
  A[4] := 42;
```
...
```pascal
procedure ProcessArr(var Arr: array of Integer);
var
  I: Integer;
begin
...
// Range checking validates indices at runtime
```
...
Using 0-based indexing on a 1-based array (or vice versa) accesses the wrong memory location or triggers a range check error.
...
**Dynamic array access after SetLength reduction**

SetLength reduces the array size. Elements beyond the new length are inaccessible and accessing them causes an access violation.
...
**Open array parameter bounds confusion**

Open array parameters are always 0-indexed. Using the original array's index range causes out-of-bounds access.
...
1. **Always enable range checking ($R+) during development and testing.**
2. **Use Low() and High() to iterate arrays instead of hard-coded index values.**
3. **Validate array indices before access, especially when the index comes from user input or external data.**
...
- [INDEX Error](/languages/pascal/pascal-index-out-of-range) — indexing problems
