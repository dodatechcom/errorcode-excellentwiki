---
title: "[Solution] Pascal String Length Error - How to Fix"
description: "Fix Pascal string overflow and length errors when string operations exceed maximum length or cause memory allocation failures."
languages: ["pascal"]
...
- Short strings (String[n]) have a fixed maximum length. Assigning a longer string truncates silently or raises a range check error depending on compiler settings.
- Long strings (AnsiString/String in modern Delphi) are heap-allocated and can grow dynamically. However, they can still exhaust memory if concatenated in loops without pre-allocation.
- Copy(), Pos(), and other string functions may return unexpected results if the string contains null characters (#0). Null-terminated strings require special handling.
- UnicodeString in Delphi 2009+ stores wide characters. Converting between AnsiString and UnicodeString may change the byte length and string content.
...
1. **Range check error - string overflow**
2. **Out of memory - string too large**
3. **String contains null characters - truncated**
4. **Unicode/Ansi conversion changes content**
...
var
  S: String[10];
begin
  S := 'This string is too long';
  // Truncated to 10 chars or range error
end;

// CORRECT: Use larger string or validate
...
begin
  S := 'Hello' + #0 + 'World';
  P := PChar(S);
  ShowMessage(string(P)); // Shows 'Hello'
end;

// CORRECT: Use Pos to find segments
```
...
begin
  A := 'Hello';
  U := A; // Implicit conversion
  // Length(U) may differ from Length(A)
end;
```
...
Short strings silently truncate when the source exceeds the maximum length. This can cause data loss without any visible warning.
...
**Memory exhaustion from string concatenation**

Repeated string concatenation in a loop creates many temporary strings, fragmenting the heap and potentially exhausting memory.
...
Strings containing #0 characters are truncated when passed to C-style functions or PChar conversions.
...
1. **Use SetLength to pre-allocate string memory before filling in loops.**
2. **Always validate string lengths before assignment to fixed-length string types.**
3. **Be aware of null character behavior when working with PChar and C-style string functions.**
...
- [HEAP Error](/languages/pascal/pascal-heap-error) — memory issues
