---
title: "[Solution] Pascal Integer Overflow Error - How to Fix"
description: "Fix Pascal integer overflow errors when arithmetic operations exceed the range of the integer type."
languages: ["pascal"]
...
- Byte (0..255), Word (0..65535), Integer (-2147483648..2147483647), and Int64 have different ranges. Operations exceeding these ranges overflow silently without {$Q+} or raise EIntOverflow with overflow checking enabled.
- Adding two large positive integers may overflow to a negative value. Subtracting two negative values may overflow to a positive value. These silent overflows cause incorrect calculations.
- With {$Q+} overflow checking, integer overflow raises EIntOverflow. Without it, the result wraps around silently, producing incorrect values.
- Multiplication of two integers can easily exceed the target range. For example, 50000 * 50000 exceeds Integer range but fits in Int64.
...
1. **EIntOverflow - integer overflow**
2. **Silent overflow - result wraps to negative**
3. **Multiplication exceeds Integer range**
4. **Addition of large positive values overflows**
...
begin
  ...
  B := 50000;
  C := A * B;
  // 2,500,000,000 > MaxInt
end;

// CORRECT: Use Int64
  C := Integer(Int64(A) * Int64(B));
```
...
**Silent integer overflow without checking**

Without {$Q+}, integer overflow wraps silently. A large positive value becomes negative, causing incorrect results without any error.
...
Multiplying two medium-sized integers can easily exceed the target type range. Always check the maximum possible result before multiplication.
...
**Accumulator overflow in loops**

A loop counter or accumulator gradually increases until it exceeds the integer range. Use a larger type for accumulators.
...
1. **Enable overflow checking ($Q+) during development and testing.**
2. **Use Int64 for accumulators and intermediate calculations that may exceed Integer range.**
3. **Validate input ranges before arithmetic operations to prevent overflow.**
...
- [DIVISION BY ZERO Error](/languages/pascal/pascal-division-by-zero) — division errors
