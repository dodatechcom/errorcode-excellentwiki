---
title: "[Solution] Pascal Inline Directive Error - How to Fix"
description: "Fix Pascal inline directive errors when functions fail to inline or cause code bloat due to incorrect inline usage."
languages: ["pascal"]
...
- The 'inline' directive suggests to the compiler that a function should be expanded at the call site. The compiler may ignore the directive if inlining would be detrimental.
- Recursive functions should not be inlined as each recursive call would be expanded, causing exponential code growth.
- Inlining small, frequently called functions improves performance by eliminating call overhead. Inlining large functions increases code size and may reduce performance due to instruction cache pressure.
- The 'inline' directive in Object Pascal is a hint, not a command. The compiler decides whether to actually inline based on function complexity and optimization settings.
...
1. **Recursive function inlined - code explosion**
2. **Inline directive ignored by compiler**
3. **Inlining large function causes cache pressure**
4. **Inline directive on virtual method ignored**
...
// CORRECT: Remove inline directive
```
...
```pascal
procedure ProcessRecord(var Rec: TRecordType);
begin
  // Large function body
  // Do not inline
end;

// CORRECT: Leave large functions uninlined
```
...
// inline is a hint only
// Compiler decides based on optimization
```
...
```pascal
function SmallFunc: Integer;
begin
  Result := 42;
end;
// Ideal candidate for inline
```
...
**Recursive function inlined**

Inlining a recursive function causes exponential code expansion. Each recursive call generates another copy of the function body.
...
Inlining a large function bloats the code size and may reduce performance due to instruction cache misses.
...
**Inline directive ignored**

The compiler may ignore the inline directive for complex functions, virtual methods, or when optimization settings do not permit inlining.
...
1. **Only use inline on small, simple, frequently called functions.**
2. **Avoid inlining recursive functions or functions with complex control flow.**
3. **Use profiling to verify that inlining actually improves performance in your specific use case.**
...
- [COMPILER Error](/languages/pascal/pascal-compiler-error) — compilation
- [RUNTIME Error](/languages/pascal/pascal-runtime-error) — runtime issues
- [OPTIMIZATION Error](/languages/pascal/pascal-optimization-error) — performance
