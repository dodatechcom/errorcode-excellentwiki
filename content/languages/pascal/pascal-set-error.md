---
title: "[Solution] Pascal Set Type Error - How to Fix"
description: "Fix Pascal set type errors including set overflow, invalid element values, and set operation mismatches."
languages: ["pascal"]
...
- Pascal sets have a maximum ordinal range of 0..255 (byte set). Attempting to create a set with ordinal values outside this range causes a compiler or runtime error.
- Set intersection, union, and difference operations require both operands to be the same set type. Mixing set types causes a type mismatch error.
- An empty set is written as []. Adding elements uses the [element] syntax. Invalid set syntax causes compilation errors.
- Sets can only contain ordinal types (Integer subtypes, Char, Boolean, enumerated). Real, string, and pointer types cannot be elements of a set.
...
1. **Set element exceeds ordinal range 0..255**
2. **Set type mismatch in operation**
3. **Invalid set element type - non-ordinal**
4. **Set constructor syntax error**
...
begin
  Days := [1, 3, 5];
  Days := Days + [8]; // OK - 8 is in range
end;

// But:
  Days := Days + [300]; // Error: out of range
```
...
begin
  Letters := ['a'..'z'];
  // Range of chars is valid
end;

// But:
  Letters := [65]; // Error: Integer, not Char
```
...
**Set element outside 0..255 range**

The ordinal value of a set element exceeds the 0..255 byte set limit. Pascal sets cannot represent values outside this range.
...
**Set type mismatch in operations**

Two different set types are used in an operation. Set operations require both operands to be the same set type.
...
**Non-ordinal type in set**

The set element type is not an ordinal type. Sets can only contain integers, characters, booleans, or enumerated values.
...
1. **Keep set element ordinal values within the 0..255 range.**
2. **Ensure both operands of set operations are the same set type.**
3. **Use only ordinal types as set elements. For larger ranges, use collections or arrays.**
