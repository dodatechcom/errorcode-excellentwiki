---
title: "[Solution] Pascal Interface Reference Error - How to Fix"
description: "Fix Pascal interface reference counting errors including circular references, nil dereference, and lifetime management issues."
languages: ["pascal"]
...
- Interface references are reference-counted. When the last interface reference goes out of scope, the object is destroyed automatically. Circular references prevent the count from reaching zero.
- An interface variable can be nil. Calling methods on a nil interface raises an EAccessViolation. Always check for nil before use.
- An interface reference keeps the underlying object alive. If the object is freed via Free while an interface still references it, subsequent interface method calls cause undefined behavior.
- Interface delegation (implements keyword) forwards interface method calls to an internal object. If the delegate object is nil or freed, interface calls fail.
...
1. **EAccessViolation - nil interface dereference**
2. **Circular reference - memory leak**
3. **Object freed while interface still references it**
4. **Interface delegation to nil delegate**
...
begin
  ...
  // Reference count = 2, correct
end;

// CORRECT: Set to nil when done
  A := nil;
  B := nil;
```
...
begin
  ...
  Intf := Obj;
  Obj.Free; // Free while Intf still valid!
  Intf.DoSomething; // Undefined behavior
end;

// CORRECT: Let interface manage lifetime
  Intf := TMyObject.Create as IInterface1;
```
...
type
  TMyObj = class(TInterfacedObject, IMyInterface)
    function GetDelegate: IDelegate;
  end;

  // Check delegation target
  function TMyObj.GetDelegate: IDelegate;
  begin
    Result := FDelegate;
    if Result = nil then
      raise Exception.Create('No delegate');
  end;
```
...
**Interface nil dereference**

An interface variable is nil when a method is called on it. This causes EAccessViolation at runtime.
...
Two objects hold interface references to each other. The reference counts never reach zero, causing memory leaks.
...
**Free called while interface exists**

The underlying object is freed manually while an interface reference still exists. The interface becomes dangling.
...
1. **Always check interface variables for nil before calling methods.**
2. **Avoid circular references between interfaces. Use weak references or events to break cycles.**
3. **Let interface reference counting manage object lifetime. Do not call Free on objects referenced by interfaces.**
