---
title: "[Solution] Pascal Infinite Loop Error - How to Fix"
description: "Fix Pascal infinite loop errors when while, for, or repeat loops never terminate due to incorrect exit conditions."
languages: ["pascal"]
...
- A while loop with a condition that never becomes False runs forever. Common mistakes include using AND instead of OR, or not modifying the loop variable.
- Repeat..Until loops execute at least once. If the Until condition is already True before entry, the loop still executes one extra iteration.
- For loops iterate a fixed number of times but may appear infinite if the range is extremely large or if the loop body takes very long.
- Infinite loops consume 100% CPU and eventually exhaust stack or memory. Detecting them requires monitoring CPU usage or adding iteration counters.
...
1. **Loop runs forever - condition never becomes False**
2. **Repeat..Until executes extra iteration**
3. **For loop with extremely large range**
4. **Loop variable not modified inside loop**
...
begin
  I := 1;
  while I > 0 do  // Always true!
  begin
    // I never changes
  end;
end;

// CORRECT: Modify loop variable
  while I > 0 do
  begin
    Dec(I);
  end;
...
begin
  ...
  repeat
    DoSomething;
  until Done;
  // Executes once even though Done is True
end;
...
begin
  ...
  begin
    // If SL is modified during loop,
    // Count changes and index may be invalid
  end;
end;

// CORRECT: Copy or iterate safely
```
...
```pascal
// Detect infinite loop with counter
var
  Counter: Integer;
begin
  Counter := 0;
  ...
  begin
    ...
    begin
      ShowMessage('Possible infinite loop');
      Break;
    end;
    DoWork;
  end;
...
The condition checks a variable that is never modified inside the loop body, causing the loop to run forever.
...
**Repeat..Until with already-True condition**

The Until condition is True before the loop starts. The loop body executes once unnecessarily.
...
Multiple conditions in the loop exit clause contain a logic error (wrong operator) that prevents the loop from terminating.
...
1. **Always ensure loop variables are modified within the loop body.**
2. **Add iteration counters with safety limits to catch infinite loops during development.**
3. **Use structured loop constructs (for..in) when the iteration count is known in advance.**
...
- [STACK OVERFLOW Error](/languages/pascal/pascal-stack-overflow) — stack issues
