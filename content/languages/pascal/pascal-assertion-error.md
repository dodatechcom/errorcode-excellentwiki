---
title: "[Solution] Pascal Assertion Error - How to Fix"
description: "Fix Pascal assertion failures when Assert statements evaluate to false, indicating violated preconditions or invariants."
languages: ["pascal"]
...
- Assert(condition, message) raises EAssertionFailed when condition evaluates to False. Assertions are debugging aids that validate assumptions about program state.
- Assertions can be disabled with compiler directives or runtime flags. Code that relies on Assert for control flow fails silently when assertions are disabled.
- Assert exceptions propagate up the call stack like any other exception. If unhandled, they terminate the program with an error message.
- In Delphi, use {$C+} to enable assertions (default in debug builds) and {$C-} to disable them in release builds.
...
1. **EAssertionFailed - assertion condition is false**
2. **Assertion disabled - validation skipped silently**
3. **Assert message not visible in release builds**
4. **Assert in time-critical code slows performance**
...
// EAssertionFailed if called with empty array
```
...
// Assertion validates precondition
```
...
```pascal
procedure ProcessItem(Item: TObject);
begin
  Assert(Item <> nil, 'Item cannot be nil');
  Assert(Item is TMyClass, 'Item must be TMyClass');
  TMyClass(Item).DoSomething;
end;
```
...
```pascal
// Use dedicated validation instead of Assert for production
procedure ValidateInput(const S: string);
begin
  if Length(S) = 0 then
    raise EInvalidInput.Create('Input cannot be empty');
end;
```
...
**Assert disabled in production**

Assertions are compiled out in release builds. If code depends on Assert for validation, the validation is silently skipped.
...
Assertions should only check programmer assumptions, not user input. Use explicit validation for user data.
...
Assert failures in production may go unnoticed if no exception handler logs them.
...
1. **Use Assert only for checking programmer assumptions, not for user input validation.**
2. **Always include meaningful messages in Assert to aid debugging.**
3. **Add explicit exception handling for production validation that must always run.**
