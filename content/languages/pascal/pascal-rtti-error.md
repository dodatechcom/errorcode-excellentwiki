---
title: "[Solution] Pascal RTTI (Run-Time Type Information) Error - How to Fix"
description: "Fix Pascal RTTI errors when using GetPropInfo, GetPropValue, or CallMethod for dynamic property and method access."
languages: ["pascal"]
...
- RTTI (Run-Time Type Information) is available only for classes compiled with {$M+} or descendants of TPersistent. Classes without RTTI cannot be inspected at runtime.
- GetPropInfo returns nil if the property does not exist. Calling GetPropValue with a nil PropInfo raises an EPropertyError.
- Published properties must be of supported types (Integer, String, TComponent, etc.). Unsupported property types cause RTTI to return incorrect data.
- Published methods must have no parameters or a single TComponent parameter. Methods with other signatures are not accessible via RTTI.
...
1. **GetPropInfo returns nil - property not found**
2. **RTTI on class without {$M+} - no type info**
3. **Published property of unsupported type**
4. **Published method with wrong signature**
...
var
  PropInfo: PPropInfo;
begin
  PropInfo := GetPropInfo(MyObj.ClassInfo, 'Name');
  if PropInfo <> nil then
    ShowMessage(GetStrProp(MyObj, PropInfo))
  else
...
```pascal
type
  TMyClass = class(TObject) // No {$M+}!
    procedure DoSomething;
  end;

// RTTI not available!

// CORRECT: Derive from TPersistent
...
// Color is not published - no RTTI
// Name is published - RTTI available
```
...
type
  ...
  published
    ...
    procedure Initialize; // Wrong signature
  end;

// Click is accessible (TComponent param)
// Initialize is not (no params allowed)
```
...
**RTTI on non-persistent class**

The class does not descend from TPersistent or have {$M+}, so RTTI information is not available at runtime.
...
**Property not found in RTTI**

GetPropInfo returns nil because the property name does not match any published property in the class.
...
**Published method with wrong signature**

The published method does not match the required signature (no params or single TComponent param) for RTTI access.
...
1. **Ensure classes descend from TPersistent to enable RTTI for property access.**
2. **Always check GetPropInfo result for nil before using the property information.**
3. **Keep published methods to the allowed signatures: no parameters or a single TComponent parameter.**
