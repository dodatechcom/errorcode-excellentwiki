---
title: "[Solution] Pascal File Locking Error - How to Fix"
description: "Fix Pascal file locking errors when exclusive access to files fails due to sharing violations or stale locks."
languages: ["pascal"]
...
- File locking prevents concurrent modification. If another process holds an exclusive lock, opening the file with fmExclusive raises an EFOpenError.
- Stale lock files may remain after a process crashes. These orphaned locks prevent other processes from accessing the file.
- Network file systems (NFS, SMB) may have delayed lock propagation, causing lock conflicts even after the lock holder has released it.
- Using fmOpenReadWrite without fmShareDenyNone may conflict with other processes that also have the file open.
...
1. **EFOpenError - sharing violation**
2. **Stale lock file after process crash**
3. **Network file lock delay**
4. **File already open with exclusive lock**
...
var
  F: TFileStream;
begin
  F := TFileStream.Create('data.bin',
    fmOpenReadWrite or fmShareDenyNone);
end;

// CORRECT: Allow shared access
  F := TFileStream.Create('data.bin',
...
```pascal
// Stale lock file remains after crash
var
  LockFile: string;
begin
  LockFile := ChangeFileExt(ParamStr(0), '.lck');
...
var
  F: TFileStream;
begin
  F := TFileStream.Create('\\server\share\file.bin',
    fmOpenRead or fmShareDenyNone);
end;

// CORRECT: Use retry logic
  Retries := 0;
  repeat
    try
...
begin
  LF := FileName + '.lck';
  while FileExists(LF) do
    Sleep(100);
  // Create lock file
  CloseHandle(FileCreate(LF));
end;
```
...
procedure UnlockFile(const FileName: string);
var
  LF: string;
begin
  LF := FileName + '.lck';
...
Opening a file with fmShareDenyAll prevents any other process from accessing it. This causes sharing violations in multi-process environments.
...
A process crashes while holding a lock file. The lock file remains, preventing other processes from acquiring the lock.
...
**Network file system lock delays**

Lock changes may not propagate immediately across network file systems, causing temporary lock conflicts.
...
1. **Use fmShareDenyWrite for read-only access and fmShareDenyNone for shared access.**
2. **Implement lock file cleanup on startup to handle stale locks from crashed processes.**
3. **Add retry logic with exponential backoff for file operations that may fail due to temporary locks.**
...
- [FILE NOT FOUND Error](/languages/pascal/pascal-file-not-found) — file access
- [I/O Error](/languages/pascal/pascal-io-error) — I/O operations
