...
description: "Fix Pascal destructor errors when freeing objects, including missing inherited Destroy, double-free, and cleanup ordering issues."
languages: ["pascal"]
...
- A destructor (Destroy) cleans up object resources. Always call inherited Destroy as the last statement to ensure proper cleanup of parent class resources.
- FreeAndNil safely frees an object and sets the reference to nil, preventing double-free. Calling Free on a nil object is safe (no-op), but calling Destroy directly on nil is not.
- Freeing an object twice (double-free) corrupts the heap and may cause access violations or data corruption. Always nil the reference after Free.
- The order of cleanup in a destructor matters. Free child objects before releasing resources they depend on.
...
1. **Missing inherited Destroy - parent cleanup skipped**
2. **Double-free - object freed twice**
3. **FreeAndNil vs Free - reference management**
4. **Destruction order matters - children freed after parents**
...
begin
  Obj := TMyClass.Create;
  Obj.Free;
  Obj.Free; // Double-free!
end;

// CORRECT: Use FreeAndNil
  FreeAndNil(Obj);
```
...
begin
  Obj := TMyClass.Create;
  FreeAndNil(Obj);
  // Obj is now nil, Free is safe
  FreeAndNil(Obj); // No-op
end;
```
...
destructor TMyClass.Destroy;
begin
  FTimer.Free; // Free timer first
  FList.Free;  // Then free list
  FConnection.Free; // Then connection
  inherited Destroy;
end;
...
The destructor does not call inherited Destroy, leaving parent class resources allocated and causing memory or resource leaks.
...
Freeing an object twice corrupts the heap. The second Free operates on freed memory, potentially corrupting other allocations.
...
Freeing objects in the wrong order may cause dependencies to be accessed after they are freed, leading to access violations.
...
1. **Always call inherited Destroy as the last statement in destructors.**
2. **Use FreeAndNil instead of Free to prevent double-free issues.**
3. **Free objects in reverse order of creation: dependents first, then their dependencies.**
...
- [CONSTRUCTOR Error](/languages/pascal/pascal-constructor-error) — constructor issues
