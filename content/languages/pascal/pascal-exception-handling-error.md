---
title: "[Solution] Pascal Exception Handling Error - How to Fix"
description: "Fix Pascal exception handling errors including swallowed exceptions, re-raise mistakes, and missing cleanup in finally blocks."
languages: ["pascal"]
...
- Swallowing exceptions with empty except blocks hides errors. The program continues in an undefined state without any indication that something went wrong.
- Raising an exception inside a finally block replaces the original exception. The original error is lost, making debugging very difficult.
- Freeing objects in except blocks without try/finally may leave other objects leaked if the exception occurs after the first Free.
- Exception messages may contain sensitive information. In production, log detailed messages but show generic messages to users.
...
1. **Empty except block - error silently ignored**
2. **Exception in finally block replaces original**
3. **Object cleanup skipped on exception**
4. **Exception message leaks sensitive data**
...
try
  ...
  FreeAndNil(Obj2);
  // If Obj2.Free raises, Obj1 is already freed
end;

// CORRECT: Free independently
  try FreeAndNil(Obj1); except end;
  try FreeAndNil(Obj2); except end;
```
...
// Nested try/finally inside try/except
```
...
```pascal
// Don't raise in finally!
try
  DoWork;
finally
  Cleanup;
  // Don't: if Error then raise
end;
```
...
An empty except block silently discards the exception. The program continues in an undefined state, and the bug is never reported or fixed.
...
**Raising in finally block**

Code in a finally block raises an exception. This replaces the original exception, making it impossible to know what the original error was.
...
When an exception occurs, resources allocated before the exception may not be freed if there is no finally block for cleanup.
...
1. **Never use empty except blocks. At minimum, log the exception message.**
2. **Always use try/finally for resource cleanup, separate from try/except for error handling.**
3. **Re-raise exceptions with 'raise' after logging. Do not create new exceptions that lose the original context.**
...
- [MEMORY Error](/languages/pascal/pascal-heap-error) — memory leaks
- [FILE NOT FOUND Error](/languages/pascal/pascal-file-not-found) — file issues
