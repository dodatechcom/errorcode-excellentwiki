---
title: "[Solution] Pascal Invalid Type Cast Error - How to Fix"
description: "Fix Pascal invalid type cast errors when converting between incompatible types using the 'as' operator or unsafe type conversions."
languages: ["pascal"]
...
- The 'as' operator in Object Pascal performs runtime type checking. If the source object is not compatible with the target type, an InvalidCastError is raised.
- Casting between incompatible types using the 'is' operator followed by 'as' is the safe pattern. Skipping the 'is' check causes runtime failures when the cast is invalid.
- Pointer type casts using the 'as' operator require that both types are class types. Casting a primitive type or interface with 'as' causes a compilation error.
- The 'as' operator cannot be used with untyped parameters or variant types. Use explicit conversion functions like StrToInt or FloatToStr instead.
...
1. **Invalid type cast - source incompatible with target**
2. **InvalidCastError at runtime - 'as' cast failed**
3. **Type cast between unrelated class types**
4. **Cast of interface to wrong interface type**
...
begin
  B := A as TComponent;
  // Error if A is not a TComponent
end;
...
begin
  ...
  (A as TMyClass).DoSomething;
  // InvalidCastError: TStringList is not TMyClass
end;

// CORRECT: Use proper type
  (A as TStringList).Add('Hello');
```
...
begin
  S := P as string;
  // Error: cannot use 'as' with pointers
end;
...
begin
  J := I as IInterface2;
  // InvalidCastError if I does not support IInterface2
end;

// CORRECT: QueryInterface first
  if Supports(I, IInterface2, J) then
    J.DoSomething;
```
...
begin
  A := nil;
  (A as TStringList).Add('Hello');
  // EInvalidCast: object is nil
end;

// CORRECT: Check for nil
  if (A <> nil) and (A is TStringList) then
    (A as TStringList).Add('Hello');
```
...
**Using 'as' without prior 'is' check**

The 'as' operator raises InvalidCastError when the object is not of the target type. Always check with 'is' first or handle the exception.
...
**Casting interfaces with 'as' fails**

Interface type casts require QueryInterface support. Not all interfaces support querying to arbitrary other interfaces.
...
**Nil object cast with 'as'**

Casting a nil object reference with 'as' does not return nil - it raises an exception.
...
1. **Always use 'is' before 'as' to check type compatibility.**
2. **Wrap 'as' casts in try/except to handle unexpected InvalidCastError gracefully.**
3. **Use Supports() function for safe interface type checking and casting.**
...
- [POINTER Error](/languages/pascal/pascal-invalid-pointer) — pointer issues
- [RUNTIME Error](/languages/pascal/pascal-runtime-error) — general runtime
- [TYPE MISMATCH Error](/languages/pascal/pascal-type-mismatch) — type conflicts
