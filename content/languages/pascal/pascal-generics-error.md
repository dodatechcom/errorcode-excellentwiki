---
title: "[Solution] Pascal Generics Type Error - How to Fix"
description: "Fix Pascal generic type errors including type constraint violations, constructor constraints, and generic instantiation failures."
languages: ["pascal"]
...
- Generic type parameters can be constrained with 'class', 'constructor', or interface constraints. Using a constrained type incorrectly causes compilation errors.
- The 'constructor' constraint requires the generic type to have a parameterless Create method. Types without a constructor cannot be used with 'new'.
- Generic specializations with incompatible types cause compilation errors. Each specialization creates a distinct type in the type system.
- Generic classes with class constraints require reference types. Using value types with 'class' constrained generics causes errors.
...
1. **Type constraint violation - generic parameter incompatible**
2. **Missing constructor constraint - cannot create instance**
3. **Generic type mismatch between specializations**
4. **Value type used with class-constrained generic**
...
var
  L: TMyList<Integer>;
  // Error: Integer is not a class
end;

// CORRECT: Use class type
  L: TMyList<TObject>;
```
...
function TFactory<T>.CreateItem: T;
begin
  Result := T.Create; // Requires constructor
end;

// Types without Create cannot be used
```
...
// CORRECT: Remove class constraint
  TCache<T> = class
```
...
The generic type parameter does not satisfy the declared constraints (class, constructor, interface). The type system rejects the instantiation.
...
**Missing constructor constraint**

The generic code calls T.Create but the type parameter lacks a 'constructor' constraint, preventing creation.
...
**Incompatible generic specializations**

Two different generic instantiations (e.g., TList<Integer> and TList<string>) are incompatible types and cannot be assigned to each other.
...
1. **Declare appropriate constraints on generic type parameters to ensure required capabilities.**
2. **Each generic specialization is a distinct type. Do not assume different specializations are compatible.**
3. **Use class constraints when the generic type needs to be a reference type. Use constructor constraints when creating instances.**
...
- [TYPE MISMATCH Error](/languages/pascal/pascal-type-mismatch) — type issues
