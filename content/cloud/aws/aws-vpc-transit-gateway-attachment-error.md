---
title: "[Solution] VPC Transit Gateway Attachment Error"
description: "Resolve Transit Gateway attachment errors when connecting VPCs across accounts."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] VPC Transit Gateway Attachment Error

Resolve Transit Gateway attachment errors when connecting VPCs across accounts.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-vpc-transit-gateway-attachment-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws vpc describe-* 2>&1 | head -50
```
