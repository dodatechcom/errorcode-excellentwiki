---
title: "[Solution] CloudFront Key Pair Error for Signed URLs"
description: "Fix CloudFront key pair errors when creating or using signed URLs and cookies."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] CloudFront Key Pair Error for Signed URLs

Fix CloudFront key pair errors when creating or using signed URLs and cookies.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-cloudfront-key-pair-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```
