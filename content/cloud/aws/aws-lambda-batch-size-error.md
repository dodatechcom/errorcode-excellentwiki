---
title: "[Solution] Lambda Event Source Mapping Batch Size Error"
description: "Resolve Lambda event source mapping batch size configuration errors for SQS and Kinesis."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] Lambda Event Source Mapping Batch Size Error

Resolve Lambda event source mapping batch size configuration errors for SQS and Kinesis.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-lambda-batch-size-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws lambda describe-* 2>&1 | head -50
```
