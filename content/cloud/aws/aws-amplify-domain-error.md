---
title: "[Solution] Amplify Custom Domain Error"
description: "Resolve Amplify custom domain errors when connecting domains to apps."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] Amplify Custom Domain Error

Resolve Amplify custom domain errors when connecting domains to apps.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-amplify-domain-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws amplify describe-* 2>&1 | head -50
```
