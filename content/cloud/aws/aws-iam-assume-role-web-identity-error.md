---
title: "[Solution] IAM Assume Role with Web Identity Error"
description: "Fix IAM AssumeRoleWithWebIdentity errors for OIDC and federated identity providers."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] IAM Assume Role with Web Identity Error

Fix IAM AssumeRoleWithWebIdentity errors for OIDC and federated identity providers.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-iam-assume-role-web-identity-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws iam describe-* 2>&1 | head -50
```
