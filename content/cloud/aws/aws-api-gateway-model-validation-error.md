---
title: "[Solution] API Gateway Model Validation Error"
description: "Fix API Gateway model validation errors when request or response does not match schema."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] API Gateway Model Validation Error

Fix API Gateway model validation errors when request or response does not match schema.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-api-gateway-model-validation-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```
