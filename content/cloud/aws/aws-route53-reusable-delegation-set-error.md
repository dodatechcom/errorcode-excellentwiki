---
title: "[Solution] Route53 Reusable Delegation Set Error"
description: "Fix Route53 reusable delegation set errors when sharing nameservers across hosted zones."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] Route53 Reusable Delegation Set Error

Fix Route53 reusable delegation set errors when sharing nameservers across hosted zones.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-route53-reusable-delegation-set-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws route53 describe-* 2>&1 | head -50
```
