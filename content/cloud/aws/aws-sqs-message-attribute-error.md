---
title: "[Solution] SQS Message Attribute Validation Error"
description: "Fix SQS message attribute validation errors when sending messages with invalid attributes."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] SQS Message Attribute Validation Error

Fix SQS message attribute validation errors when sending messages with invalid attributes.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-sqs-message-attribute-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws sqs describe-* 2>&1 | head -50
```
