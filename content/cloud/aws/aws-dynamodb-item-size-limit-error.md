---
title: "[Solution] DynamoDB Item Too Large Error"
description: "Fix DynamoDB item size limit errors when putting or updating items that exceed 400 KB."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] DynamoDB Item Too Large Error

Fix DynamoDB item size limit errors when putting or updating items that exceed 400 KB.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-dynamodb-item-size-limit-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```
