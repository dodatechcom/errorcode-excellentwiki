---
title: "[Solution] Athena Workgroup Error"
description: "Resolve Athena workgroup errors when configuring query execution settings."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] Athena Workgroup Error

Resolve Athena workgroup errors when configuring query execution settings.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-athena-workgroup-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws athena describe-* 2>&1 | head -50
```
