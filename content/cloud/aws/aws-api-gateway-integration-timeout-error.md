---
title: "[Solution] API Gateway Integration Timeout Error"
description: "Fix API Gateway 504 Gateway Timeout errors when backend integration takes too long."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] API Gateway Integration Timeout Error

Fix API Gateway 504 Gateway Timeout errors when backend integration takes too long.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-api-gateway-integration-timeout-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```
