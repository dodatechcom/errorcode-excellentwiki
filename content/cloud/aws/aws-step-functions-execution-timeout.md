---
title: "[Solution] Step Functions Execution Timeout Error"
description: "Fix Step Functions execution timeout errors when state machine runs too long."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] Step Functions Execution Timeout Error

Fix Step Functions execution timeout errors when state machine runs too long.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-step-functions-execution-timeout   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws step describe-* 2>&1 | head -50
```
