---
title: "[Solution] RDS Snapshot Share Across Accounts Error"
description: "Fix RDS snapshot sharing errors when sharing manual snapshots between AWS accounts."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] RDS Snapshot Share Across Accounts Error

Fix RDS snapshot sharing errors when sharing manual snapshots between AWS accounts.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-rds-snapshot-shared-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```
