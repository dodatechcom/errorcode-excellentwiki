---
title: "[Solution] SNS Message Filter Policy Error"
description: "Fix SNS message filter policy errors when subscriptions do not receive expected messages."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] SNS Message Filter Policy Error

Fix SNS message filter policy errors when subscriptions do not receive expected messages.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-sns-message-filter-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws sns describe-* 2>&1 | head -50
```
