---
title: "[Solution] AWS Config Compliance Error"
description: "Resolve AWS Config compliance errors when evaluating resource compliance rules."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] AWS Config Compliance Error

Resolve AWS Config compliance errors when evaluating resource compliance rules.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-config-compliance-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws config describe-* 2>&1 | head -50
```
