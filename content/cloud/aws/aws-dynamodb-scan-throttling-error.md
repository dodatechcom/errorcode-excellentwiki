---
title: "[Solution] DynamoDB Scan Throttling Error"
description: "Fix DynamoDB scan throttling errors when full table scans exceed provisioned capacity."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] DynamoDB Scan Throttling Error

Fix DynamoDB scan throttling errors when full table scans exceed provisioned capacity.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-dynamodb-scan-throttling-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws dynamodb describe-* 2>&1 | head -50
```
