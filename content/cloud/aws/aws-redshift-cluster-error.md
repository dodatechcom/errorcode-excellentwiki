---
title: "[Solution] Redshift Cluster Connection Error"
description: "Fix Amazon Redshift cluster errors when connecting to data warehouse instances."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] Redshift Cluster Connection Error

Fix Amazon Redshift cluster errors when connecting to data warehouse instances.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-redshift-cluster-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws redshift describe-* 2>&1 | head -50
```
