---
title: "[Solution] API Gateway Mapping Template Error"
description: "Resolve API Gateway mapping template errors when transforming request and response data."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] API Gateway Mapping Template Error

Resolve API Gateway mapping template errors when transforming request and response data.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-api-gateway-mapping-template-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws api describe-* 2>&1 | head -50
```
