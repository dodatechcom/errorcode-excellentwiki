---
title: "[Solution] Secrets Manager Cross-Account Error"
description: "Resolve Secrets Manager cross-account errors when sharing secrets between accounts."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] Secrets Manager Cross-Account Error

Resolve Secrets Manager cross-account errors when sharing secrets between accounts.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-secrets-manager-cross-account-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws secrets describe-* 2>&1 | head -50
```
