---
title: "[Solution] SES Identity Not Verified Error"
description: "Fix SES identity verification errors when sending from unverified addresses."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] SES Identity Not Verified Error

Fix SES identity verification errors when sending from unverified addresses.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-ses-identity-not-verified   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws ses describe-* 2>&1 | head -50
```
