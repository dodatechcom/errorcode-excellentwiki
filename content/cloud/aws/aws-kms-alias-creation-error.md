---
title: "[Solution] KMS Alias Creation Error"
description: "Resolve KMS alias creation errors when creating friendly names for KMS keys."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] KMS Alias Creation Error

Resolve KMS alias creation errors when creating friendly names for KMS keys.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-kms-alias-creation-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws kms describe-* 2>&1 | head -50
```
