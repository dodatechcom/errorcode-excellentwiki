---
title: "[Solution] CloudFront Multi-Origin Configuration Error"
description: "Resolve CloudFront multi-origin errors when configuring multiple origin servers."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] CloudFront Multi-Origin Configuration Error

Resolve CloudFront multi-origin errors when configuring multiple origin servers.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-cloudfront-multi-origin-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws cloudfront describe-* 2>&1 | head -50
```
