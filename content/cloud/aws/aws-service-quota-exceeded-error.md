---
title: "[Solution] AWS Service Quota Exceeded Error"
description: "Resolve AWS service quota exceeded errors when resource limits are reached."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] AWS Service Quota Exceeded Error

Resolve AWS service quota exceeded errors when resource limits are reached.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-service-quota-exceeded-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws service describe-* 2>&1 | head -50
```
