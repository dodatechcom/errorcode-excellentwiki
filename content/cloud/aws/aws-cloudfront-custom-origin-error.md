---
title: "[Solution] CloudFront Custom Origin Error"
description: "Fix CloudFront custom origin errors when distribution fails to connect to origin servers."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] CloudFront Custom Origin Error

Fix CloudFront custom origin errors when distribution fails to connect to origin servers.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-cloudfront-custom-origin-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```
