---
title: "[Solution] CodePipeline Stage Failure Error"
description: "Fix CodePipeline stage failure errors when pipeline stages fail to execute."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] CodePipeline Stage Failure Error

Fix CodePipeline stage failure errors when pipeline stages fail to execute.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-codepipeline-stage-failure   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws codepipeline describe-* 2>&1 | head -50
```
