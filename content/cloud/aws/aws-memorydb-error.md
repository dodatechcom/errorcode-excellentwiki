---
title: "[Solution] MemoryDB for Redis Error"
description: "Fix MemoryDB for Redis errors when creating or managing in-memory database clusters."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] MemoryDB for Redis Error

Fix MemoryDB for Redis errors when creating or managing in-memory database clusters.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-memorydb-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws memorydb describe-* 2>&1 | head -50
```
