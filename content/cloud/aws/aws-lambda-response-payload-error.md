---
title: "[Solution] Lambda Response Payload Too Large Error"
description: "Fix Lambda response payload size limit errors when returning data from functions."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] Lambda Response Payload Too Large Error

Fix Lambda response payload size limit errors when returning data from functions.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-lambda-response-payload-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```
