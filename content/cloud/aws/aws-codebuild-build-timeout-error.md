---
title: "[Solution] CodeBuild Build Timeout Error"
description: "Fix CodeBuild build timeout errors when builds exceed configured time limits."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] CodeBuild Build Timeout Error

Fix CodeBuild build timeout errors when builds exceed configured time limits.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-codebuild-build-timeout-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws codebuild describe-* 2>&1 | head -50
```
