---
title: "[Solution] AppSync Schema Error"
description: "Fix AWS AppSync schema errors when defining GraphQL schemas."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] AppSync Schema Error

Fix AWS AppSync schema errors when defining GraphQL schemas.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-appsync-schema-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws appsync describe-* 2>&1 | head -50
```
