---
title: "[Solution] Glue ETL Job Error"
description: "Resolve AWS Glue ETL job errors when running data transformation workflows."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] Glue ETL Job Error

Resolve AWS Glue ETL job errors when running data transformation workflows.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-glue-job-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws glue describe-* 2>&1 | head -50
```
