---
title: "[Solution] OpenSearch Domain Error"
description: "Resolve Amazon OpenSearch Service domain errors when managing search clusters."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] OpenSearch Domain Error

Resolve Amazon OpenSearch Service domain errors when managing search clusters.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-opensearch-domain-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws opensearch describe-* 2>&1 | head -50
```
