---
title: "[Solution] RDS IAM Database Authentication Error"
description: "Fix RDS IAM database authentication errors when connecting using IAM credentials."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] RDS IAM Database Authentication Error

Fix RDS IAM database authentication errors when connecting using IAM credentials.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-rds-custom-iam-auth-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```
