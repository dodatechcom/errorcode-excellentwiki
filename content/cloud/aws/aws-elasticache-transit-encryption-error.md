---
title: "[Solution] ElastiCache Transit Encryption Error"
description: "Resolve ElastiCache transit encryption errors when enabling in-transit encryption."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] ElastiCache Transit Encryption Error

Resolve ElastiCache transit encryption errors when enabling in-transit encryption.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-elasticache-transit-encryption-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws elasticache describe-* 2>&1 | head -50
```
