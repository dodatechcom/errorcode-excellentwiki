---
title: "[Solution] RDS Aurora Cluster Parameter Group Error"
description: "Resolve RDS Aurora cluster parameter group errors when modifying database parameters."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] RDS Aurora Cluster Parameter Group Error

Resolve RDS Aurora cluster parameter group errors when modifying database parameters.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-rds-cluster-parameter-group-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws rds describe-* 2>&1 | head -50
```
