---
title: "[Solution] SNS Subscription Pending Confirmation Error"
description: "Fix SNS subscription pending errors when subscriptions are not confirmed."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] SNS Subscription Pending Confirmation Error

Fix SNS subscription pending errors when subscriptions are not confirmed.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-sns-subscription-pending-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```
