---
title: "[Solution] EC2 AMI Encryption Mismatch Error"
description: "Resolve EC2 AMI encryption errors when launching instances from encrypted AMIs across regions."
cloud: ["aws"]
error-types: ["cloud-error"]
---

# [Solution] EC2 AMI Encryption Mismatch Error

Resolve EC2 AMI encryption errors when launching instances from encrypted AMIs across regions.
...
```bash
aws logs filter-log-events   --log-group-name /aws/aws-ec2-ami-encryption-error   --start-time $(date -d '1 hour ago' +%s)000   --filter-pattern "ERROR"
```

### Enable Detailed Error Logging

```bash
aws ec2 describe-* 2>&1 | head -50
```
